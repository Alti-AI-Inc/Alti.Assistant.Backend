...
import { sanitizeQueryString, getPremiumV17IntelligenceData } from '../src/app/helpers/v17DataIntegrations.js';
import { altiPremiumIntelligenceSearch } from '../src/app/modules/search/tools.js';

async function runTests() {
  console.log('🚀 Running Alti.Assistant v17 Premium Legal, Regulatory & Threat Intelligence Data Integrations Test Suite...');
  let failures = 0;
  ...
  console.log('\n🧪 [Test 2] Multi-lingual Script Preservation');
  const multilingualQuery = 'Legal Opinion (中国法律) & Sanctions.';
  ...
  if (sanitizedMulti.includes('中国法律') && sanitizedMulti.includes('Legal') && sanitizedMulti.includes('Sanctions') && !sanitizedMulti.includes('&')) {
  ...
  console.log('\n🧪 [Test 3] High-Performance Benchmarks (Execution time < 500ms or warm cached < 50ms) for All 6 Domains');
  const domains = [
    'courtlistener',
    'harvard_caselaw',
    'cisa_kev',
  ...
  for (const domain of domains) {
    ...
    if (domain === 'harvard_caselaw') query = 'Anderson';
    else if (domain === 'cisa_kev') query = 'CVE-2024-21345';
    else if (domain === 'nist_nvd_cve') query = 'CVE-2024-12345';
    else if (domain === 'ofac_sanctions') query = 'Dmitry Petrov';
    else if (domain === 'fara_foreign_agents') query = 'Alpine Strategies';
  ...
  console.log('✅ Test 3 Passed: Benchmark complete and bolding formatting verified across all 6 domains.');

  // Test 4: Windows-Resilient Dual-Layer Cache Verification
  console.log('\n🧪 [Test 4] Windows-Resilient Dual-Layer Cache Verification');
  const cacheKeyTestDomain = 'cisa_kev';
  const cacheKeyTestQuery = 'CVE-2024-9999';
  
  // First lookup (cache miss, populates cache)
  const t0 = Date.now();
  await getPremiumV17IntelligenceData(cacheKeyTestDomain, cacheKeyTestQuery);
  const d0 = Date.now() - t0;
  console.log(`- First lookup (cold): ${d0}ms`);
  
  // Second lookup (cache hit from memory)
  const t1 = Date.now();
  const cachedResult = await getPremiumV17IntelligenceData(cacheKeyTestDomain, cacheKeyTestQuery);
  ...
  if (!schema) {
    ...
    // Validate each of the 6 domains through the Zod schema
    for (const domain of domains) {
  ...
  console.log(`\n========================================`);
  if (failures === 0) {
    console.log('🟢 ALL v17 Premium PUBLIC INTELLIGENCE INTEGRATION TESTS PASSED SUCCESSFULLY! 🟢');
    process.exit(0);
  } else {
