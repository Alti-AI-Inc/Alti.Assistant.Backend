...
function loadProductsFromConfig() {
  ...
  if (!fs.existsSync(configPath)) {
    throw new Error(
      `Config file not found: ${configPath}. Please run 'npm run seed:stripe' first.`
    );
  }
...
async function seedProductsToDatabase() {
  try {
    ...
    const productsToInsert = plans.map((plan) => ({
      ...
      description:
        plan.description || `${plan.name} plan with advanced features`,
      price: plan.price,
      ...
      features: {
        ...
        researchLimit: plan.features.researchLimit || 0,
        imageLimit: plan.features.imageLimit || 0,
        videoLimit: plan.features.videoLimit || 0,
        taskLimit: plan.features.taskLimit || 0,
        workflowLimit: plan.features.workflowLimit || 0,
        searchLimit: plan.features.searchLimit || 0,
        writeLimit: plan.features.writeLimit || 0,
        codeLimit: plan.features.codeLimit || 0,
        projectsLimit: plan.features.projectsLimit || 0,
        modelsLimit: plan.features.modelsLimit || 0,
        knowledgeLimit: plan.features.knowledgeLimit || 0,
      },
    ...
    for (const product of insertedProducts) {
      ...
      console.log(
        `     • Storage: ${(product.features.storagePerUser / 1073741824).toFixed(0)}GB/user`
      );
      console.log(
        `     • Team Invites: ${product.features.canInviteTeam ? 'Yes' : 'No'}`
      );
