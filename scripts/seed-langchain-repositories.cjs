...
async function seed() {
  if (!fs.existsSync(CATALOG_PATH)) {
    ...
    console.error(`Please run the scanner script first: node scripts/scan-langchain-repos.cjs\n`);
    process.exit(1);
  }
  ...
  console.log(`Clearing existing LangchainRepository documents...`);
  
  const deleteResult = await LangchainRepository.deleteMany({});
  ...
  await LangchainRepository.collection.dropIndexes().catch(() => {});
  console.log(`✓ Dropped stale indexes.`);

  console.log(`Seeding database with new vetted repositories...`);

  // Insert documents in bulk
  const insertResult = await LangchainRepository.insertMany(catalog);
  console.log(`✓ Successfully seeded ${insertResult.length} Langchain repositories!`);
