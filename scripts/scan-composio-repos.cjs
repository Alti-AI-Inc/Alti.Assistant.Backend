...
// Configuration
const ORG = 'ComposioHQ';
const OUTPUT_DIR = path.join(__dirname, '../output');
const CATALOG_PATH = path.join(OUTPUT_DIR, 'composio-license-catalog.json');
...
function githubRequest(endpoint) {
  return new Promise((resolve, reject) => {
    const options = {
      ...
      path: endpoint,
      headers: {
        'User-Agent': 'Alti-Assistant-Composio-Scanner',
...
// Fetch and filter all Composio repositories
async function scanComposioOrg() {
  let repos = [];
  let page = 1;
  const perPage = 100;
  const catalog = [];
  ...
  console.log(`\n======================================================`);
  console.log(`SUCCESS! Composio organization scan finished.`);
