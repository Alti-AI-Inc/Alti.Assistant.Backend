...
const LOG_PATH = path.join(__dirname, '../logs/composio-install.log');
const ERROR_LOG_PATH = path.join(__dirname, '../logs/composio-install-error.log');
...
async function main() {
  if (!fs.existsSync(CATALOG_PATH)) {
    logError('Composio repository catalog not found. Please run the scanner first.');
    process.exit(1);
  }

  log('======================================================');
  log('Starting Monolithic Composio Submodule Installation Task');
  log('======================================================');
  ...
  // Ensure external/composio directory exists
  ...
  for (let i = 0; i < catalog.length; i++) {
    ...
    const index = i + 1;
    const submodulePath = `external/composio/${repo.name}`;
  ...
  log('======================================================');
  log('Composio Installation task completed!');
