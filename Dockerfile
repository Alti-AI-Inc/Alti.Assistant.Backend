...
WORKDIR /app/insoai-core-service
...
COPY --from=builder /app/insoai-core-service/node_modules ./node_modules

# Copy package files (needed for Node.js module resolution)
COPY package*.json ./

# Copy only necessary application files from builder
COPY --from=builder /app/insoai-core-service/src ./src
COPY --from=builder /app/insoai-core-service/config ./config
COPY --from=builder /app/insoai-core-service/shared ./shared
# test/ intentionally excluded from production image
COPY --from=builder /app/insoai-core-service/index.js ./
COPY --from=builder /app/insoai-core-service/package.json ./
# insoai_gcp.json excluded from image — mount at runtime via secret volume
COPY --from=builder /app/insoai-core-service/imagegen.json ./
# env.yaml is gitignored (secrets); Cloud Run injects vars via --set-env-vars/--set-secrets
COPY --from=builder /app/insoai-core-service/preload.cjs ./

# Create necessary directories
RUN mkdir -p logs/errors logs/successes uploads/ragsystem uploads/mcp_toolbox storage/ragsystem output

# Run as non-root user for security
RUN chown -R node:node /app/insoai-core-service
USER node
