...
COPY --from=builder /app/alti-backend/node_modules ./node_modules

# Copy package files (needed for Node.js module resolution)
COPY package*.json ./

# Copy only necessary application files from builder
COPY --from=builder /app/alti-backend/src ./src
COPY --from=builder /app/alti-backend/config ./config
COPY --from=builder /app/alti-backend/shared ./shared
# test/ intentionally excluded from production image
COPY --from=builder /app/alti-backend/index.js ./
COPY --from=builder /app/alti-backend/package.json ./
COPY --from=builder /app/alti-backend/preload.cjs ./

# Create necessary directories
RUN mkdir -p logs/errors logs/successes uploads/ragsystem uploads/mcp_toolbox storage/ragsystem output

# Run as non-root user for security
RUN chown -R node:node /app/alti-backend
USER node
