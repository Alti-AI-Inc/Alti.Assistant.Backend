/**
 * @fileoverview Video Agent microservice entry point.
...
const { logger } = createLogger('agent-video');
...
async function start() {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Video Agent listening on port ${PORT}`, {
        env: config.env,
        port: PORT,
      });
    });
  } catch (error) {
    logger.error(`Failed to start Video Agent: ${error.message}`);
    process.exit(1);
  }
}
