...
export const OpenStackService = {
   ...
   * OpenStack Zun (Container Service v1)
   * Boots ultra-fast, air-gapped ephemeral containers for AI Code Execution (Code Interpreter).
   * Runs untrusted Python/Node.js code generated by the LLM in complete isolation.
   */
  async executeAirGappedCode(code, language = 'python') {
    if (!process.env.OS_USERNAME) return null;
    logger.info(`[OpenStack Zun] Booting air-gapped container for ${language} execution`);
    ...
    try {
      ...
      const image = language === 'python' ? 'python:3.11-slim' : 'node:20-slim';
      const command = language === 'python' ? ['python', '-c', code] : ['node', '-e', code];

      // 1. Create the container
      const createRes = await axios.post(`${zunEndpoint}/v1/containers`, {
        name: containerName,
        image: image,
        command: command,
        memory: 512, // MB
        cpu: 1.0,
        auto_remove: true, // Self-destruct when execution finishes
        interactive: false
      }, {
      ...
      await axios.post(`${zunEndpoint}/v1/containers/${containerUuid}/actions`, {
        start: {}
      }, {
        headers: { 'X-Auth-Token': auth.token, 'OpenStack-API-Version': 'container 1.12' }
      });

      // 3. (Async) Logs would be retrieved via Zun logs API. 
      // For synchronous responses, Zun supports websockets or polling.
      return {
        containerId: containerUuid,
        status: 'EXECUTING',
        message: 'Code sandbox deployed to Liberty Center One edge.'
      };
    } catch (error) {
