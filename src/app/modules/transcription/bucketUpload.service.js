 ...
 * Uses credentials specified in `config.google.google_application_credentials` or a default `insoai_gcp.json` file.
 * The project ID is also configured via `config.google.gcp_project_id`.
 */
const storage = new Storage({
  keyFilename:
    config.google?.google_application_credentials || './insoai_gcp.json',
 ...
 * Defaults to 'insoai_assistant_transcription' if not specified in `config.gcs.transcription_bucket`.
 */
const BUCKET_NAME = config.gcs?.transcription_bucket || 'insoai_assistant_transcription';
