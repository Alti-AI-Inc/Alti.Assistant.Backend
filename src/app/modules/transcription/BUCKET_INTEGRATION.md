...
Audio files uploaded for transcription are now **automatically saved to the `inso_assistant_transcription` GCP bucket** for permanent storage. This allows you to:
...
User uploads audio → Save to GCS bucket → Use gs:// URI in Gemini → Process → Store results
                     ↓
                  gs://inso_assistant_transcription/transcriptions/
...
**Bucket:** `inso_assistant_transcription` (Google Cloud Storage)  
**Access:** Private (requires signed URLs for external access)  
**Naming:** `transcriptions/{timestamp}-{originalName}`  
**GCS URI:** `gs://inso_assistant_transcription/transcriptions/{filename}`  
**Public URL:** `https://storage.googleapis.com/inso_assistant_transcription/transcriptions/{filename}`
...
```json
{
  "metadata": {
    ...
    "gsUri": "gs://inso_assistant_transcription/transcriptions/1732723200-meeting.mp3",
    "gcsFileName": "transcriptions/1732723200-meeting.mp3",
    "bucketName": "inso_assistant_transcription",
...
//   gsUri: 'gs://inso_assistant_transcription/transcriptions/meeting.mp3',
...
```javascript
{
  gsUri: 'gs://inso_assistant_transcription/transcriptions/...',  // GCS URI for Gemini
...
const bucketUrl = `gs://your-gcs-bucket/inso_assistant_transcription/${filename}`;
