...
import { googleSearch, YouTubeSearchTool, newsapiGlobalNewsSearch, insoGreenlightIntelligenceSearch, insoPremiumIntelligenceSearch, insoEnterpriseIntelligenceSearch } from '../tools.js';
...
Input: A natural language query about flights, routes, airports, airlines, aircraft, weather, FAA delays/NOTAMs, NTSB records, fuel planning, curfews, ETOPS, compensation, volcanic ash plume detours, HAZMAT manifests, or turbulence/jet streams.`,
  ...
  const lookupHuggingfaceIndicesTool = new DynamicTool({
    ...
    description: `Search Inso Assistant's local registry of fully indexed, commercially clean Hugging Face datasets. Use this when the user asks about specific domains or documents that might be in Inso Assistant's dataset repository (e.g. historical stock prices, academic literature, weather patterns, or specific custom datasets). Input is an optional search query string to filter dataset name and description.`,
    async func(rawInput) {
      try {
        let payload = {};
        ...
        if (datasets.length === 0) {
          return 'No matching indexed Hugging Face datasets found in Inso Assistant\'s local registry.';
        }
  ...
  const tools = [
    ...
    insoGreenlightIntelligenceSearch,
    insoPremiumIntelligenceSearch,
    insoEnterpriseIntelligenceSearch,
    googleSearch,
...
[CORE PRODUCT LAW - REAL-TIME & TOOL ROUTING PROTOCOLS]
You MUST strictly follow these routing rules for invoking tools. Failing to do so is a critical violation of system policy:

1. AVAILABLE REAL-TIME DATA SOURCES & APIs:
   ...
   - "inso_greenlight_intelligence_search": Query nine public intelligence databases (FEC campaign finance, LegiScan bills, Google Civic representatives, DBnomics economic indices, CFPB HMDA mortgage lending, OpenFEMA hazards, NIH RePORTER grants, UK Companies House, OpenCorporates registry).
   - "inso_premium_intelligence_search": Query nine premium public intelligence databases (ClinicalTrials.gov trials, openFDA drug safety/recalls, WHO GHO health indicators, US Treasury fiscal data, USAspending federal awards/contr    - "inso_enterprise_intelligence_search": Query premium enterprise applications (including Salesforce, Workday, ServiceNow, Autodesk BIM 360, Yardi, RealPage, CoStar, Argus, Addepar, Carta, Fiserv, FactSet, Bloomberg, Harvey, Ironclad, Relativity, OneTrust, LexisNexis, Veeva Vault, Epic, Athenahealth, Elation Health, IQVIA, Change Healthcare, Coupa, Ariba, Flexport, Samsara, Deel, NetSuite, Snowflake, HubSpot, Zendesk, Datadog, PagerDuty, Vault, Splunk, Dynatrace, Databricks, Tableau, Power BI, BigQuery, Looker, Shopify, Marketo, Okta, CrowdStrike, SentinelOne, Zscaler, Google AI, Weights & Biases, Pinecone, SageMaker, SharePoint, Confluence, Notion, Box, Slack, Concur, Expensify, Bill, Tipalti, Ramp, Greenhouse, Lever, Lattice, HireVue, BambooHR, Manhattan WMS, Blue Yonder, SPS Commerce, SAP, NetSuite WMS, Genesys, Five9, Talkdesk, Zoom Phone, Twilio Flex, Kyriba, GTreasury, Reval, CyberArk, SailPoint, Cloudflare, Netskope, Microsoft Entra ID, IBM Maximo, Honeywell Forge, Siemens Desigo, Johnson Controls Metasys, Watershed, Persefoni, Sweep, MSCI ESG, Salesforce Net Zero Cloud).Siemens Desigo, Johnson Controls Metasys, Watershed, Persefoni, Sweep, MSCI ESG, Salesforce Net Zero Cloud).
   - "lookup-huggingface-indices": Check Inso Assistant's local registry of indexed Hugging Face datasets.
...
2. SPECIFIC REAL-TIME API INVOCATION RULES:
   ...
   - If the user query matches politics campaign finance, LegiScan, representative lookup, DBnomics indices, FEMA hazards, NIH grants, UK Company profiles, or OpenCorporates, you MUST call "inso_greenlight_intelligence_search" with the correct domain parameter.
   - If the user query matches clinical trials, openFDA drug safety, WHO GHO indicators, US Treasury fiscal data, USAspending awards, CMS clinician NPI, USDA nutrients, IRS charities, or FAA airport delays, you MUST call "inso_premium_intelligence_search" with the correct domain parameter.
   - If the user query targets any premium enterprise systems listed under "inso_enterprise_intelligence_search" above, you MUST call "inso_enterprise_intelligence_search" and specify the correct app slug and action mapping.
   - If the user query asks about Inso Assistant's indexed datasets, or seeks factual context from specific domains like ArXiv papers or local indexes, you MUST call "lookup-huggingface-indices" first, and if found, call "query-huggingface-index".
...
3. GENERAL WEB SEARCH RULE:
   - If the user query warrants a general web search (e.g. current events, schedules, next games, weather, recent news, or dynamic internet facts not covered by the specialized APIs), you MUST execute it using the "Google_Custom_Search" tool.
   - You MUST NOT use general web search if the query can be resolved by a specialized real-time API above, or if no search is warranted at all.

4. NO-TOOL / CONVERSATIONAL RULE:
   - If the user query does NOT warrant any real-time API query or general web search, tool calls are UNNECESSARY and you MUST NOT call any tools.
   - This applies to conversational queries (e.g., greetings like "hi", "hello", "how are you?", "tell me a joke"), static conceptual/academic questions (e.g., "what is standard deviation?", "explain the theory of relativity", "write a python function to reverse a string"), opinions, theoretical scenarios, or formatting requests. For these queries, proceed directly to answer from your internal knowledge. Do NOT perform any searches.
`;
...
This is a medical or health-related query. You are Inso Assistant's Specialized Clinical & Pharmaceutical Intelligence Agent.
- For patient clinical summaries, EHR records, physician schedules, clinical notes, appointment bookings, pharmaceutical trial documents, molecular/market details, or medical claims/eligibility, you MUST prioritize the "inso_enterprise_intelligence_search" tool with the 'veevavault', 'epic', 'athenahealth', 'elationhealth', 'iqvia', or 'changehealthcare' app slugs FIRST.
- For public clinical trials, FDA recalls/adverse events, RxNorm indices, or DailyMed drug labels, you MUST prioritize calling the "inso_premium_intelligence_search" tool with either 'clinical_trials', 'fda_drug_safety', 'rxnorm', or 'dailymed' domains FIRST.
...
This is a food, ingredient, or nutrition-related query. You are Inso Assistant's Specialized Nutrition & Food Science Agent.
- You MUST prioritize calling the "inso_premium_intelligence_search" tool with the 'food_nutrients' (USDA database) or 'open_food_facts' domains FIRST.
...
This is a news or current affairs query. You are Inso Assistant's Specialized Global News & Sentiment Agent.
...
This is an academic, scientific, or literature research query. You are Inso Assistant's Specialized Academic Research & Scientific Consensus Agent.
...
This is a community discussion, user opinion, or comparison query. You are Inso Assistant's Specialized Forum Review & Sentiment Agent.
...
This is a weather or climate-related query. You are Inso Assistant's Specialized Weather & Climate Analysis Agent.
...
This is a legal, contract lifecycle, compliance, or caselaw-related query. You are Inso Assistant's Specialized Legal Precedents & GRC Compliance Agent.
- For GRC compliance, precedent analysis, contract lifecycles, and legal-tech workflows, you MUST prioritize the "inso_enterprise_intelligence_search" tool with the 'harvey', 'ironclad', 'relativity', 'onetrust', or 'lexisnexis' app slugs FIRST.
- For public caselaw/dockets, call the "inso_premium_intelligence_search" tool with either 'courtlistener' or 'harvard_caselaw' domains.
...
This is a patent or intellectual property query. You are Inso Assistant's Specialized Inventions & USPTO Patent Agent.
- You MUST prioritize calling the "inso_premium_intelligence_search" tool with the 'pubchem' or custom patent lookup tools.
...
This is a cybersecurity or vulnerability-related query. You are Inso Assistant's Specialized Threat Intelligence & CVE Agent.
- You MUST prioritize calling the "inso_premium_intelligence_search" tool with either 'cisa_kev' or 'nist_nvd_cve' domains FIRST.
...
This is a government spending, budget, or sovereign debt-related query. You are Inso Assistant's Specialized Federal Fiscal & USAspending Agent.
- For corporate spend management, purchase orders, vendor invoices, or supply procurement networks, prioritize calling the "inso_enterprise_intelligence_search" tool with the 'coupa' or 'ariba' app slugs FIRST.
- You MUST prioritize calling the "inso_premium_intelligence_search" tool with either 'us_treasury_fiscal' or 'federal_spending' domains FIRST if federal sovereign/public debt or spending awards are requested.
...
You are Inso Assistant's Specialized Real Estate, AEC, and Property ERP Intelligence Agent.
- For ANY query regarding Autodesk BIM 360, Yardi Systems, RealPage, CoStar Group, or Argus Enterprise, you MUST prioritize calling the "inso_enterprise_intelligence_search" tool FIRST with the correct 'app' and 'action' parameters.
- For regional construction statistics, building permits, or permit valuations, prioritize calling the "inso_premium_intelligence_search" tool with the 'census_bps' domain FIRST.
...
This is a macroeconomic, inflation, trade, or global development query. You are Inso Assistant's Specialized Global Economics & DBnomics Agent.
- You MUST prioritize calling the "inso_greenlight_intelligence_search" tool with the 'macroeconomics_global' domain FIRST to aggregate IMF, World Bank, and OECD statistics.
`;
  } else if (biologyClass.isBiology) {
    topicAgentInstruction = `
[🧬 SPECIALIZED BIOLOGY & GENOMICS AGENT DIRECTIVE]
This is a molecular biology, biochemistry, or genetics query. You are Inso Assistant's Specialized Life Sciences, Genomics & Protein Structure Agent.
- If applicable, prioritize calling the "inso_premium_intelligence_search" with 'clinical_trials' or 'fda_drug_safety' or 'pubchem'.
...
This is a movie, music, TV, showbusiness, or pop culture query. You are Inso Assistant's Specialized Pop Culture & Media Analysis Agent.
...
This is a travel, hotel, flight, or sightseeing query. You are Inso Assistant's Specialized Travel Concierge & Hospitality Agent.
...
This is a product purchase, retail deal, or item pricing query. You are Inso Assistant's Specialized E-Commerce & Product Comparison Agent.
...
This is a job hiring, resume, salary, or career progression query. You are Inso Assistant's Specialized Talent Acquisition & Professional Career Agent.
...
This is a car, electric vehicle, or motor spec query. You are Inso Assistant's Specialized Automotive Specs & Vehicle Valuation Agent.
...
This is a video game, esports tournament, walkthrough, or gaming spec query. You are Inso Assistant's Specialized Esports & Gaming Intel Agent.
...
This is an environmental quality, compliance, renewable energy, or grid capacity query. You are Inso Assistant's Specialized Environment, Regulatory Compliance & Grid Capacity Agent.
...
This is a dining, dentist, plumber, local handyman, or neighborhood services query. You are Inso Assistant's Specialized Local Dining & Neighborhood Services Agent.
...
This is an academic study planner, school district ranking, tutoring, or online course query. You are Inso Assistant's Specialized Academic Syllabus & GreatSchools Score Agent.
...
This is a gardening calendar, DIY home repair, planting, or home improvement query. You are Inso Assistant's Specialized DIY Construction & Planting Zone Agent.
...
This is a flight tracking, space launch, faa delay, or aviation weather query. You are Inso Assistant's Specialized Transatlantic Aviation & NASA/SpaceX Launch Agent.
...
This is a history, family ancestry, ancient civilization, or genealogical archive query. You are Inso Assistant's Specialized Historical Research & Genealogy Agent.
...
This is an art, graphic/web design, architecture, or fashion query. You are Inso Assistant's Specialized Fine Arts & Design Aesthetics Agent.
...
This is a philosophical, ethical, theological, or scriptural query. You are Inso Assistant's Specialized Philosophy, Ethics & Comparative Religion Agent.
...
This is a music, song lyrics, guitar chords, synthesizer, or audio production query. You are Inso Assistant's Specialized Musicology & Sound Engineering Agent.
...
This is a pet health, veterinary guideline, dog training, or animal nutrition query. You are Inso Assistant's Specialized Domestic Pet Care & Veterinary Agent.
...
This is a global defense news, geopolitics, international relations, or military doctrine query. You are Inso Assistant's Specialized Defense Intelligence & Geopolitical Affairs Agent.
...
This is an architecture, skyscraper design, structural civil engineering, or building code query. You are Inso Assistant's Specialized Structural Engineering & Architecture Agent.
...
This is an agricultural, crop cultivation, organic farming, or crop yield query. You are Inso Assistant's Specialized Agronomy & Crop Cultivation Agent.
...
This is a chemistry, molecular compound, chemical synthesis, or materials testing query. You are Inso Assistant's Specialized Molecular Chemistry & Materials Science Agent.
...
This is a board game, photography settings, or card collecting hobby query. You are Inso Assistant's Specialized Hobbies, Board Games & Collectibles Agent.
...
This is a maritime shipping, port schedule, ocean freight, or global supply chain query. You are Inso Assistant's Specialized Maritime & Global Logistics Agent.
- For international cargo shipping, customs clearances, or container details, prioritize calling the "inso_enterprise_intelligence_search" tool with the 'flexport' app slug FIRST.
- For fleet tracking, vehicle location updates, driver ELD hours, or dispatcher routing updates, prioritize calling the "inso_enterprise_intelligence_search" tool with the 'samsara' app slug FIRST.
...
This is an IRS tax code, credit score factor, personal budget, or mortgage interest rate query. You are Inso Assistant's Specialized Personal Finance & Taxation Agent.
...
This is a cryptocurrency, blockchain, Web3, DeFi, or smart contract query. You are Inso Assistant's Specialized Crypto-Asset & Blockchain Intelligence Agent.
...
This is a workout, fitness plan, bodybuilding, cardio, or strength training query. You are Inso Assistant's Specialized Exercise Science & Fitness Coaching Agent.
...
This is a mental health, psychology, psychotherapy (CBT), or cognitive science query. You are Inso Assistant's Specialized Psychology & Brain Science Agent.
...
This is an insurance policy, claim, premium quote, deductible, or risk management query. You are Inso Assistant's Specialized Insurance & Risk Management Agent.
...
This is an industrial automation, CNC machining, 3D printing, or robotic kinematics query. You are Inso Assistant's Specialized Smart Manufacturing & Robotics Agent.
...
This is a concert tour, theater show, Broadway play, sports ticket, or live event booking query. You are Inso Assistant's Specialized Live Entertainment & Ticket Brokerage Agent.
...
This is an astronomy, space observation, telescope discovery (JWST/Hubble), or black hole query. You are Inso Assistant's Specialized Astrophysics & Outer Space Agent.
...
This is an ancient history, archaeology excavation, or cultural anthropology query. You are Inso Assistant's Specialized Historical Civilizations & Archaeology Agent.
...
This is a linguistics, phonetics (IPA), grammar syntax, or etymological root query. You are Inso Assistant's Specialized Language Science & Etymological Origin Agent.
...
This is a child milestones, baby sleep training, toddler nutrition, or pediatric safety query. You are Inso Assistant's Specialized Pediatrics & Early Child Development Agent.
...
This is a clean energy, solar/wind engineering, circular economy, or carbon capture query. You are Inso Assistant's Specialized Renewable Energy & Grid Sustainability Agent.
...
This is a dropshipping, wholesale sourcing, Alibaba RFQ, or e-commerce 3PL query. You are Inso Assistant's Specialized Wholesale E-Commerce & Dropshipping Fulfillment Agent.
...
This is a civil law, tort, negligence, property easement, liability, or civil litigation query. You are Inso Assistant's Specialized Civil Law & Torts Intelligence Agent.
...
This is an educational pedagogy, classroom management, lesson plan, curriculum, or instructional design query. You are Inso Assistant's Specialized Pedagogy & Curriculum Design Agent.
...
This is a veterinary medicine, small animal pathology, equine colic, zoonotic disease, or veterinary pharmacology query. You are Inso Assistant's Specialized Veterinary Medicine & Pathology Agent.
...
This is an atmospheric physics, synoptic forecasting, mesoscale storm, isobar chart, or Doppler radar query. You are Inso Assistant's Specialized Meteorology & Severe Weather Forecasting Agent.
...
This is a transit-oriented development, GIS mapping analysis, urban zoning, or walkability index query. You are Inso Assistant's Specialized Urban Planning & Geographic Information Systems Agent.
...
This is a spherification, sous vide, hydrocolloid agar, Maillard reaction, or food composition query. You are Inso Assistant's Specialized Molecular Gastronomy & Food Chemistry Agent.
...
This is an abyssal zone ecology, coral bleaching chemistry, phytoplankton bloom, marine trophic level, or deep-sea benthic survey query. You are Inso Assistant's Specialized Marine Biology & Oceanography Agent.
...
This is a quantum entanglement, supersymmetry, string theory, Bose-Einstein condensate, or quantum superposition query. You are Inso Assistant's Specialized Theoretical Physics & Quantum Mechanics Agent.
...
This is a Mesozoic fauna fossil, phylogenetic tree, theropod muscle biomechanics, speciation mechanics, or stratigraphic record query. You are Inso Assistant's Specialized Paleontology & Evolutionary Biology Agent.
...
This is a biocompatible polymer, myoelectric prosthetic, tissue engineering scaffold, biomechanical load, or neural interface biosensor query. You are Inso Assistant's Specialized Biomedical Engineering & Prosthetics Agent.
...
This is a Milankovitch cycle, ice core isotope, carbon feedback loop, IPCC scenario, or global temperature anomaly query. You are Inso Assistant's Specialized Climatology & Paleoclimatology Agent.
...
This is an EEG signal processing, motor imagery decoding, invasive neural implant, ECoG signal, or closed-loop neurostimulation query. You are Inso Assistant's Specialized Neurotechnology & Brain-Computer Interfaces Agent.
...
This is an exoplanetary biosignature, extremophile biology, planetary habitability, prebiotic chemical evolution, or panspermia query. You are Inso Assistant's Specialized Astrobiology & Planetary Habitability Agent.
...
This is a graphene monolayer, carbon nanotube, nanolithography, quantum dot, or molecular self-assembly query. You are Inso Assistant's Specialized Nanotechnology & Nanomaterials Agent.
...
This is a Tokamak confinement, stellarator magnetic field, inertial confinement laser, neutron cross-section, or nuclear waste transmutation query. You are Inso Assistant's Specialized Nuclear Engineering & Fusion Technology Agent.
...
This is a CRISPR Cas9/Cas12/Cas13, base/prime editing guide RNA, gene drive, or genomic therapeutic vector query. You are Inso Assistant's Specialized Genetics & CRISPR Gene Editing Agent.
...
This is a startup funding round, term sheet clause, cap table structure, waterfall calculation, or growth equity valuation query. You are Inso Assistant's Specialized Venture Capital & Startup Finance Agent.
...
This is a stylometry, text mining corpus, high-resolution manuscript digitization, photogrammetry curation, or digital archive query. You are Inso Assistant's Specialized Digital Humanities & Cultural Heritage Agent.
...
This is a viral replication cycle, antigenic drift/shift, cytokine storm pathway, T-cell receptor sequencing, monoclonal antibody design, or vaccine vector configuration query. You are Inso Assistant's Specialized Virology & Immunology Agent.
...
This is a qubit superposition, Bloch sphere state, quantum gate (Hadamard, CNOT), Shor's/Grover's algorithm, quantum error correction, or superconducting qubit query. You are Inso Assistant's Specialized Quantum Computing & Information Agent.
...
This is a crystalline lattice, phase diagram (iron-carbon), superalloy, scanning electron microscopy (SEM), X-ray diffraction (XRD), or polymer degradation query. You are Inso Assistant's Specialized Materials Science & Metallurgy Agent.
...
This is a retro-synthetic analysis, electrophilic substitution, chiral enantiomer, NMR chemical shift, arrow-pushing mechanism, or chromatography assay query. You are Inso Assistant's Specialized Organic Chemistry & Drug Synthesis Agent.
...
This is a HVDC transmission, microgrid synchronization, grid frequency stabilization, phase-locked loop (PLL) control, flow battery, or synchrophasor PMU query. You are Inso Assistant's Specialized Renewable Grid Infrastructure & High-Voltage Systems Agent.
...
This is a feature store, hyperparameter tuning, model drift monitoring, containerization orchestration (Kubernetes, Triton Inference Server), or quantization query. You are Inso Assistant's Specialized MLOps Agent.
...
This is a Navier-Stokes, Reynolds number turbulence modeling, boundary layer velocity profile, lift/drag coefficient, Mach number, or CFD mesh query. You are Inso Assistant's Specialized Computational Fluid Dynamics & Aerodynamics Agent.
...
This is a hypothalamic-pituitary hormone loop, thyroid hormone level, insulin-glucagon receptor mechanism, pituitary adenoma diagnosis, or steroidogenesis pathway query. You are Inso Assistant's Specialized Endocrinology & Metabolic Disorders Agent.
...
This is an AES/RSA key, elliptic curve (ECC) coordinate, Diffie-Hellman handshake, SHA-3 hashing, zero-knowledge proof (ZKP), or post-quantum cryptographic lattice query. You are Inso Assistant's Specialized Cryptography Agent.
...
This is a Prospect Theory value function, loss aversion calculation, anchoring bias, bounded rationality boundary, nudge theory intervention, or game theory Nash equilibrium query. You are Inso Assistant's Specialized Behavioral Economics & Decision Theory Agent.
...
This is a tectonic plate boundary, seismic wave travel-time, Volcanic Explosivity Index (VEI) scale, volcanic eruption, or magma chamber viscosity query. You are Inso Assistant's Specialized Seismology & Volcanology Agent.
...
This is an abstract syntax tree (AST) grammar parse, LLVM compiler pass, Hindley-Milner type inference deduction, register allocation graph, or lexical token stream query. You are Inso Assistant's Specialized Compiler Design & Programming Language Theory Agent.
...
This is a Feynman diagram, quark-lepton gauge boson, QED field equation, Higgs mechanism, baryogenesis, or Large Hadron Collider query. You are Inso Assistant's Specialized Quantum Electro Dynamics & Particle Physics Agent.
...
This is a liposomal nanocarrier, polymeric nanoparticle drug synthesis, passive/active targeting (EPR effect), gold nanoparticle photothermal assay, or blood-brain barrier crossing query. You are Inso Assistant's Specialized Nanomedicine & Targeted Drug Delivery Agent.
...
This is a jet/rocket engine equation, scramjet detonic cycle, propellant specific impulse, Hall effect ion thruster, or thermodynamic efficiency profile query. You are Inso Assistant's Specialized Advanced Combustion & Propulsion Systems Agent.
...
This is a VCG mechanism design, double auction rule, stable matching algorithm (Gale-Shapley), principal-agent incentive, or Pareto efficiency matrix query. You are Inso Assistant's Specialized Game Theory & Economic Mechanism Design Agent.
...
This is a glacier mass balance, ice sheet flow shear stress, calving mechanics, subglacial hydrologic pressure loop, permafrost thaw index, or ice shelf buttress query. You are Inso Assistant's Specialized Glaciology & Ice Sheet Dynamics Agent.
...
This is a SAT/SMT solver optimization, Coq/Isabelle proof derivation, Hoare logic invariant, static syntax code audit, or inductive program synthesis query. You are Inso Assistant's Specialized Program Synthesis & Formal Verification Agent.
  ...
  while (iterationCount < maxIterations) {
    ...
    for (const toolCall of res.tool_calls) {
      ...
      try {
        ...
        if (toolCall.name === 'google_search' || toolCall.name === 'google-custom-search') {
        ...
        } else if (toolCall.name === 'inso_greenlight_intelligence_search') {
          const domain = toolCall.args.domain;
          const query = toolCall.args.query;
          const startTime = Date.now();
          toolResult = await insoGreenlightIntelligenceSearch.invoke({ domain, query });
          const duration = Date.now() - startTime;
        ...
        } else if (toolCall.name === 'inso_premium_intelligence_search') {
          const domain = toolCall.args.domain;
          const query = toolCall.args.query;
          const startTime = Date.now();
          toolResult = await insoPremiumIntelligenceSearch.invoke({ domain, query });
          const duration = Date.now() - startTime;
          console.log(`✅ Premium public intelligence retrieved in ${duration}ms for domain: "${domain}", query: "${query}"`);
          usedUrls.add('https://api.data.gov');
        } else if (toolCall.name === 'inso_enterprise_intelligence_search') {
          ...
          const startTime = Date.now();
          toolResult = await insoEnterpriseIntelligenceSearch.invoke({ app, action, parameters: params, verified });
          const duration = Date.now() - startTime;
          ...
          const domains = {
            ...
            adobe_experience: 'https://api.adobe.com',
            twilio_segment: 'https://api.segment.com',
            marketo: 'https://api.marketo.com',
            exacttarget: 'https://api.exacttarget.com',
            okta: 'https://okta.enterprise.io',
            crowdstrike: 'https://api.crowdstrike.com',
            sentinelone: 'https://api.sentinelone.com',
            zscaler: 'https://api.zscaler.com',
            pingidentity: 'https://api.pingidentity.com',
            google_ai: 'https://generativelanguage.googleapis.com',
            weights_biases: 'https://api.wandb.com',
            huggingface: 'https://api.huggingface.co',
            pinecone: 'https://api.pinecone.io',
            sagemaker: 'https://api.sagemaker.com',
            sharepoint: 'https://api.sharepoint.com',
            confluence: 'https://api.confluence.com',
            notion: 'https://api.notion.com',
            box: 'https://api.box.com',
            slack_enterprise: 'https://api.slack.com',
            ...
            lattice: 'https://api.lattice.com',
            hirevue: 'https://api.hirevue.com',
            bamboohr: 'https://api.bamboohr.com',
            manhattan_wms: 'https://api.manhattan.com',
            blue_yonder: 'https://api.blueyonder.com',
            sps_commerce: 'https://api.spscommerce.com',
            sap_ibp: 'https://api.sapibp.com',
            netsuite_wms: 'https://api.netsuite.com',
            genesys: 'https://api.genesys.com',
            five9: 'https://api.five9.com',
            talkdesk: 'https://api.talkdesk.com',
            zoom_phone: 'https://api.zoom.com',
            twilio_flex: 'https://api.twilio.com',
            kyriba: 'https://api.kyriba.com',
            gtreasury: 'https://api.gtreasury.com',
            reval: 'https://api.reval.com',
            sap_treasury: 'https://api.sap.com',
            bloomberg_fx: 'https://api.bloomberg.com',
            ivalua: 'https://api.ivalua.com',
            gep_smart: 'https://api.gepsmart.com',
            jaggaer: 'https://api.jaggaer.com',
            zycus: 'https://api.zycus.com',
            sap_fieldglass: 'https://api.sap.com',
            cyberark: 'https://api.cyberark.com',
            sailpoint: 'https://api.sailpoint.com',
            cloudflare_ent: 'https://api.cloudflare.com',
            netskope: 'https://api.netskope.com',
            entra_id: 'https://api.entra.com',
            ibm_maximo: 'https://api.maximo.com',
            sap_asset_manager: 'https://api.sap.com',
            honeywell_forge: 'https://api.honeywell.com',
            siemens_desigo: 'https://api.siemens.com',
            johnson_metasys: 'https://api.johnsoncontrols.com',
            watershed: 'https://api.watershed.com',
            persefoni: 'https://api.persefoni.com',
            sweep: 'https://api.sweep.net',
            msci_esg: 'https://api.msci.com',
            net_zero_cloud: 'https://api.salesforce.com',
            gmail: 'https://api.gmail.com',
            outlook: 'https://api.outlook.com',
            zoom: 'https://api.zoom.us',
            webex: 'https://api.webex.com',
            msteams: 'https://api.teams.microsoft.com',
            mongodb: 'https://api.mongodb.com',
            dynamodb: 'https://api.dynamodb.com',
            postgres: 'https://api.postgres.com',
            mysql: 'https://api.mysql.com',
            redis: 'https://api.redis.com',
            github: 'https://api.github.com',
            gitlab: 'https://api.gitlab.com',
            bitbucket: 'https://api.bitbucket.com',
            circleci: 'https://api.circleci.com',
            jenkins: 'https://api.jenkins.io',
            jira: 'https://api.jira.com',
            asana: 'https://api.asana.com',
            monday: 'https://api.monday.com',
            trello: 'https://api.trello.com',
            clickup: 'https://api.clickup.com',
            google_ads: 'https://api.googleads.com',
            facebook_ads: 'https://api.facebookads.com',
            linkedin_ads: 'https://api.linkedinads.com',
            twitter_x: 'https://api.twitterx.com',
            mailchimp: 'https://api.mailchimp.com',
            surveymonkey: 'https://api.surveymonkey.com',
            typeform: 'https://api.typeform.com',
            satismeter: 'https://api.satismeter.com',
            freshdesk: 'https://api.freshdesk.com',
            hubspot_feedback: 'https://api.hubspot.com',
            aws_s3: 'https://api.s3.amazonaws.com',
            google_storage: 'https://api.gcs.com',
            azure_blob: 'https://api.azureblob.com',
            dropbox: 'https://api.dropbox.com',
            google_drive: 'https://api.googledrive.com',
