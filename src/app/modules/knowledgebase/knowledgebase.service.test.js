...
const {
  ...
  const mockChatbotFindOne = vi.fn();
...
vi.mock('./knowledgebase.model.js', () => {
  ...
  MockModel.find = mockKnowledgeBaseFind;
  MockModel.findOne = mockKnowledgeBaseFindOne;
  MockModel.deleteOne = mockKnowledgeBaseDeleteOne;
  return {
    default: MockModel,
...
vi.mock('./knowledgebase.files.model.js', () => {
  ...
  MockModel.find = mockKnowledgebaseFileFind;
  MockModel.findOne = mockKnowledgebaseFileFindOne;
  MockModel.deleteOne = mockKnowledgebaseFileDeleteOne;
  return {
    default: MockModel,
...
vi.mock('../chatbots/chatbot.model.js', () => {
  const MockModel = vi.fn(function (data) {
    Object.assign(this, data);
  });
  MockModel.findOne = mockChatbotFindOne;
  return {
    default: MockModel,
    findOne: mockChatbotFindOne,
  };
});
...
vi.mock('@google/generative-ai/server', () => ({
  GoogleAIFileManager: vi.fn().mockImplementation(function () {
    return {
...
describe('KnowledgebaseService', () => {
  ...
  beforeEach(() => {
    ...
    // Reset all mock functions to clear once-resolved queues
    mockGetFile.mockReset();
    mockGenerateContent.mockReset();
    mockUploadFile.mockReset();
    mockDeleteFile.mockReset();
    mockRagAddDocumentFromBuffer.mockReset();
    mockRagQuery.mockReset();
    mockFileSave.mockReset();
    mockChatbotFindOne.mockReset();
    ...
    mockChatbotFindOne.mockReturnValue({ lean: vi.fn().mockResolvedValue(null) });
  ...
  describe('uploadToGCS', () => {
    ...
    const expectedPublicUrl = expect.stringMatching(
      new RegExp(`https://storage.googleapis.com/inso_assistant_knowledge_bot_files/${mockKnowledgebotId}/\\d+_test\\.pdf`)
    );
    ...
    it('should upload a file to GCS successfully and return public URL', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining(`Uploading file to GCS: inso_assistant_knowledge_bot_files/`)
      );
      expect(mockStorageBucket).toHaveBeenCalledWith('inso_assistant_knowledge_bot_files');
      ...
      expect(result).toEqual(expectedPublicUrl);
    });
  ...
  describe('processUploadedFile', () => {
    ...
    const mockGcsUrl = 'https://storage.googleapis.com/inso_assistant_knowledge_bot_files/kb-456/123_document.txt';
    ...
    it('should process a media file from path, extract content, upload to GCS, add transcription to RAG, and save file record', async () => {
      ...
      const mediaGcsUrl = 'https://storage.googleapis.com/inso_assistant_knowledge_bot_files/kb-456/123_image.png';
    ...
    it('should handle media extraction returning non-JSON or parsing error', async () => {
      ...
      const mediaGcsUrl = 'https://storage.googleapis.com/inso_assistant_knowledge_bot_files/kb-456/123_audio.mp3';
      knowledgebaseService.uploadToGCS.mockResolvedValueOnce(mediaGcsUrl);
