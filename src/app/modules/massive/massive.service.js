  ...
  if (
    ...
    return {
      ...
      workspaceId:
        ...
        'global_default_workspace',
    };
  }

  return {
    userId: 'global_default_user',
    workspaceId: 'global_default_workspace',
  };
};
...
const getClient = () => {
  if (!_rest) {
    ...
    if (!apiKey) {
      logger.warn(
        '[Massive.com] MASSIVE_API_KEY not set. Real-time data unavailable.'
      );
    }
...
const withUsageTracking = async (
  ...
  if (!normalizedUserContext.workspaceId) {
    logger.error(
      `[Massive] Unauthorized API call attempt: ${functionName}. Missing userContext.`
    );
    throw new AppError(
      'Unauthorized: User context is required for this operation.',
      401
    );
  }

  // HIERARCHY: Check if the user/workspace has enough API credits. This respects limits set by admins.
  const canCall = await usageService.canMakeApiCall(
  ...
  if (!canCall) {
    logger.warn(
      `[Massive] API limit exceeded for user ${userContext.userId} in workspace ${userContext.workspaceId}.`
    );
    throw new AppError(
      'API call limit exceeded for your plan. Please upgrade or wait.',
      429
    );
  }
  ...
  try {
    ...
    await usageService.recordApiCall(normalizedUserContext, {
      service: 'massive',
    ...
    logger.error(
      `[Massive] API call failed for ${functionName} (User: ${normalizedUserContext.userId}, Workspace: ${normalizedUserContext.workspaceId}):`,
      error
    );
...
const fmtTickerList = (tickers) =>
  (Array.isArray(tickers) ? tickers : String(tickers).split(','))
    .map(fmt)
    .join(',');
...
const getStockQuoteService = async (userContext, ticker) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Stock Quote: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(
    ...
    4
  ); // This operation consists of 4 API calls.
};
...
const getStocksSnapshotTickersService = async (userContext, tickers) => {
  ...
  logger.info(
    `[Massive] Stocks Snapshot Tickers: ${tickerStr} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(
    userContext,
    'getStocksSnapshotTickersService',
    async () => {
      const response = await getClient().getStocksSnapshotTickers({
        tickers: tickerStr,
      });
...
const getUniversalSnapshotService = async (userContext, tickers) => {
  const tickerStr = fmtTickerList(tickers);
  logger.info(
    `[Massive] Universal Snapshot: ${tickerStr} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(
    userContext,
    'getUniversalSnapshotService',
    async () => {
      const response = await getClient().getSnapshots({
        'ticker.any_of': tickerStr,
        limit: 10,
      });
...
const getTickerDetailsService = async (userContext, ticker) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Ticker Details: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
...
const getPreviousCloseService = async (userContext, ticker) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Previous Close: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getPreviousCloseService', async () => {
    const response = await getClient().getPreviousStocksAggregates({
      stocksTicker: t,
    });
...
const getStockAggregatesService = async (userContext, params) => {
  ...
  logger.info(
    `[Massive] Stock Aggregates: ${t} (${dateFrom} to ${dateTo}) (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
...
const getStockFinancialsRatiosService = async (userContext, ticker) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Stock Financials Ratios: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(
    userContext,
    'getStockFinancialsRatiosService',
    async () => {
      const response = await getClient().getStocksFinancialsV1Ratios({
        ticker: t,
        limit: 1,
      });
      return response?.results?.[0] || response;
...
const getStockIncomeStatementService = async (userContext, ticker) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Income Statement: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(
    userContext,
    'getStockIncomeStatementService',
    async () => {
      const response = await getClient().getStocksFinancialsV1IncomeStatements({
        ticker: t,
        limit: 4,
      });
      return response?.results || response;
...
const getStockBalanceSheetsService = async (userContext, ticker) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Balance Sheet: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(
    userContext,
    ...
    async () => {
      const [bs, cf] = await Promise.allSettled([
        ...
        getClient().getStocksFinancialsV1CashFlowStatements({
          ticker: t,
          limit: 2,
        }),
      ]);
...
const getDividendsService = async (userContext, ticker) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Dividends: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getDividendsService', async () => {
    const response = await getClient().getStocksV1Dividends({
      ticker: t,
      limit: 8,
    });
    return response?.results || response;
...
const getStockSplitsService = async (userContext, ticker) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Stock Splits: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getStockSplitsService', async () => {
    const response = await getClient().getStocksV1Splits({
      ticker: t,
      limit: 5,
    });
    return response?.results || response;
...
const getShortInterestService = async (userContext, ticker) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Short Interest: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getShortInterestService', async () => {
    const response = await getClient().getStocksV1ShortInterest({
      ticker: t,
      limit: 5,
    });
    return response?.results || response;
...
const getStockFloatService = async (userContext, ticker) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Stock Float: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
...
const getStockRSIService = async (userContext, ticker, window = 14) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Stock RSI: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
...
const getStockMACDService = async (userContext, ticker) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Stock MACD: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
...
const getStockEMAService = async (userContext, ticker, window = 50) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Stock EMA-${window}: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
...
const getStockSMAService = async (userContext, ticker, window = 50) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Stock SMA-${window}: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
...
const getStockTechnicalSnapshotService = async (userContext, ticker) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Stock Technicals: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
...
const getStockNewsService = async (userContext, ticker, limit = 5) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Stock News: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getStockNewsService', async () => {
    const response = await getClient().listNews({
      ticker: t,
      limit: Number(limit),
    });
    return response?.results || response;
 ...
 */
const getOptionsChainService = async (
  userContext,
  underlyingTicker,
  limit = 30
) => {
  ...
  logger.info(
    `[Massive] Options Chain: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
 ...
 */
const getOptionsChainFilteredService = async (
  userContext,
  ticker,
  { expiration, type, limit = 20 } = {}
) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Options Chain Filtered: ${t} exp=${expiration} type=${type} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  ...
  return withUsageTracking(userContext, 'getOptionsChainFilteredService', () =>
    getClient().getOptionsChain(params)
  );
};
...
const listOptionsContractsService = async (userContext, ticker, limit = 20) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] List Options Contracts: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(
    userContext,
    'listOptionsContractsService',
    async () => {
      const response = await getClient().listOptionsContracts({
        underlying_ticker: t,
        limit: Number(limit),
      });
      return response?.results || response;
...
const getCryptoSnapshotService = async (userContext, tickers) => {
  const tickerStr = fmtTickerList(tickers);
  logger.info(
    `[Massive] Crypto Snapshot: ${tickerStr} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(
    userContext,
    'getCryptoSnapshotService',
    async () => {
      const response = await getClient().getCryptoSnapshotTickers({
        tickers: tickerStr,
      });
      return response?.tickers || response;
    }
...
const getCryptoTradesService = async (userContext, cryptoTicker, limit = 3) => {
  logger.info(
    `[Massive] Crypto Trades: ${cryptoTicker} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getCryptoTradesService', async () => {
    const response = await getClient().getCryptoTrades({
      cryptoTicker,
      limit: Number(limit),
    });
    return response?.results || response;
 ...
 */
const getCryptoAggregatesService = async (
  userContext,
  ticker,
  { timespan = 'day', limit = 7 } = {}
) => {
  logger.info(
    `[Massive] Crypto Aggregates: ${ticker} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
...
const getCryptoRSIService = async (userContext, ticker, window = 14) => {
  logger.info(
    `[Massive] Crypto RSI: ${ticker} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
...
const getCryptoMACDService = async (userContext, ticker) => {
  logger.info(
    `[Massive] Crypto MACD: ${ticker} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
...
const getCryptoEMAService = async (userContext, ticker, window = 50) => {
  logger.info(
    `[Massive] Crypto EMA-${window}: ${ticker} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
...
const getCryptoTechnicalSnapshotService = async (userContext, ticker) => {
  logger.info(
    `[Massive] Crypto Technicals: ${ticker} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(
    userContext,
...
const getForexSnapshotService = async (userContext, tickers) => {
  const tickerStr = fmtTickerList(tickers);
  logger.info(
    `[Massive] Forex Snapshot: ${tickerStr} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getForexSnapshotService', async () => {
    const response = await getClient().getForexSnapshotTickers({
      tickers: tickerStr,
    });
    return response?.tickers || response;
  });
 ...
 */
const getCurrencyConversionService = async (
  userContext,
  from,
  to,
  amount = 1
  ...
  logger.info(
    `[Massive] Currency Conversion: ${from}->${to} x${amount} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getCurrencyConversionService', () =>
    getClient().getCurrencyConversion({
 ...
 */
const getForexAggregatesService = async (
  userContext,
  pair,
  { timespan = 'day', limit = 7 } = {}
) => {
  logger.info(
    `[Massive] Forex Aggregates: ${pair} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  const to = new Date().toISOString().split('T')[0];
  const from = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
    .toISOString()
...
const getFedInflationService = async (userContext, limit = 12) => {
  logger.info(
    `[Massive] Fed Inflation (CPI) (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getFedInflationService', () =>
    getClient().getFedV1Inflation({ limit: Number(limit) })
  );
};
...
const getFedYieldsService = async (userContext, limit = 5) => {
  logger.info(
    `[Massive] Fed Treasury Yields (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getFedYieldsService', () =>
    getClient().getFedV1TreasuryYields({ limit: Number(limit) })
  );
};
...
const getFedLaborMarketService = async (userContext, limit = 3) => {
  logger.info(
    `[Massive] Fed Labor Market (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getFedLaborMarketService', () =>
    getClient().getFedV1LaborMarket({ limit: Number(limit) })
  );
};
...
const getFedInflationExpectationsService = async (userContext, limit = 3) => {
  logger.info(
    `[Massive] Fed Inflation Expectations (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(
    userContext,
    'getFedInflationExpectationsService',
    () => getClient().getFedV1InflationExpectations({ limit: Number(limit) })
  );
};
...
const getMarketStatusService = async (userContext) => {
  logger.info(
    `[Massive] Global Market Status (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getMarketStatusService', () =>
    getClient().getMarketStatus()
  );
};
...
const getMarketHolidaysService = async (userContext) => {
  logger.info(
    `[Massive] Global Market Holidays (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getMarketHolidaysService', () =>
    getClient().getMarketHolidays()
  );
};
...
const getMarketNewsService = async (userContext, ticker, limit = 5) => {
  logger.info(
    `[Massive] Market News: ${ticker || 'general'} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
...
const getIPOsService = async (userContext, limit = 10) => {
  logger.info(
    `[Massive] IPO Listings (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getIPOsService', async () => {
    try {
      ...
      logger.warn(
        `[Massive] IPOs not available for user ${userContext.userId}: ${e.message}`
      );
      return [];
    }
  });
...
const getStock52WeekService = async (userContext, ticker) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] 52-Week High/Low: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getStock52WeekService', async () => {
    ...
    if (results.length === 0)
      return { ticker: t, week52High: null, week52Low: null };
    ...
    return {
      ...
      pctFromHigh: latest?.c
        ? (((latest.c - high52) / high52) * 100).toFixed(2)
        : null,
      pctFromLow: latest?.c
        ? (((latest.c - low52) / low52) * 100).toFixed(2)
        : null,
    };
  });
};
...
const getTopMoversService = async (userContext, direction = 'gainers') => {
  ...
  logger.info(
    `[Massive] Top Movers: ${direction} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(userContext, 'getTopMoversService', async () => {
    const response = await getClient().getStocksSnapshotTickers({
      tickers: stockOnly.join(','),
    });
...
const getMarketNewsGlobalService = async (userContext, limit = 8) => {
  logger.info(
    `[Massive] Global Market News (limit=${limit}) (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(
    userContext,
...
const getDividendDetailService = async (userContext, ticker, limit = 4) => {
  const t = fmt(ticker);
  logger.info(
    `[Massive] Dividend Detail: ${t} (User: ${userContext?.userId}, Workspace: ${userContext?.workspaceId})`
  );
  return withUsageTracking(
    userContext,
 ...
 */
const getShortInterestDetailService = async (
  userContext,
  ticker,
  limit = 3
) => {
