 ...
 */
export const processWorkflowRequest = async (userPrompt, userId, conversationId = null) => {
  try {
 ...
 */
export const continueWorkflowConversation = async (userInput, conversationId, userId) => {
  try {
    logger.info(`Continuing workflow conversation ${conversationId} for user ${userId}`);
    ...
    return {
      ...
      error: error.message,
      result: {
        response: "I apologize, but I encountered an error continuing our conversation. Please try starting a new workflow request.",
        responseType: "error"
      }
    };
  }
