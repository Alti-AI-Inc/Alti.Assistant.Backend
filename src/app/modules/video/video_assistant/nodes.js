...
export const getVideoConfirmationNode = async (state) => {
  ...
  const message = "I think I have a good amount of detail now. Should I proceed with generating the video, or is there anything else you'd like to add?";
  return {
...
export const compileVideoFinalPromptNode = async (state) => {
  ...
  const message = "Great! I've created a detailed prompt based on our conversation. Now generating your video, this may take a few minutes...";
  return {
    finalPrompt,
    responseMessage: message,
    conversationHistory: [{ type: 'ai', message }],
...
export const routeVideoInitial = (state) => {
  ...
  console.log("Video Conversation History Length:", state.conversationHistory?.length || 0);
