...
const exaWebsetsRequest = async (method, path, body) => {
  const apiKey = process.env.EXA_API_KEY || process.env.EXA_KEY || config.exa_api_key;
  if (!apiKey || apiKey.includes('your_exa') || apiKey.includes('dummy')) {
    return { ok: false, status: 503, body: { message: 'EXA_API_KEY is not configured' } };
  }
  ...
  try {
    ...
    return { ok: false, status: 502, body: { message: `Exa Websets request failed: ${error.message}` } };
  }
  ...
  try {
    ...
    return { ok: false, status: 502, body: { message: 'Exa returned an invalid JSON response.' } };
  }
