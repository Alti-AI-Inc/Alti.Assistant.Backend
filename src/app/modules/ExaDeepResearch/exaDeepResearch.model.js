...
import {
  EXA_SEARCH_STATUS,
  EXA_SEARCH_TYPE,
} from './exaDeepResearch.contant.js';
...
const exaSearchSchema = new Schema(
  {
    ...
    searchType: {
      type: String,
      enum: EXA_SEARCH_TYPE, // now includes 'deep-reasoning'
      default: 'auto',
    },
    category: {
      type: String,
    ...
    // NEW: mirrors Exa's `outputSchema` request param (e.g. { type: "text" },
    // or a full JSON schema for structured extraction). Left schemaless
    // since its shape depends entirely on what the caller asks Exa for.
    outputSchema: {
      type: Schema.Types.Mixed,
    },
    // Raw parameters sent to Exa (numResults, includeDomains,
    // excludeDomains, startPublishedDate, useAutoprompt, outputSchema, etc.).
    ...
    // NEW: verbatim Exa response body. Added alongside `deep-reasoning`
    // support because that mode's response shape (reasoning/analysis
    // content driven by `outputSchema`) isn't fully covered by
    // `resultItemSchema` yet — this guarantees nothing gets lost while the
    // normalizer catches up to whatever fields Exa actually returns for it.
    rawResponse: {
      type: Schema.Types.Mixed,
    },
...
export const ExaSearch = model('Deep-Research', exaSearchSchema);
