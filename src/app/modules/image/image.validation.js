...
const imageGenerationSchema = z.object({
  body: z.object({
    ...
    }).min(3, 'Image prompt must be at least 3 characters').max(2000, 'Image prompt too long'),
    conversationId: z.string().optional(),
    imageSize: z.enum(['small', 'standard', 'large']).optional(),
    imageStyle: z.enum(['realistic', 'cartoon', 'abstract', 'photorealistic']).optional(),
...
const imageAnalysisSchema = z.object({
  body: z.object({
    ...
    conversationId: z.string().optional(),
    analysisType: z.enum(['describe', 'extract_text', 'detect_objects', 'identify_style', 'compare']).optional(),
  }),
});
...
const imagePreferencesSchema = z.object({
  body: z.object({
    ...
    style: z.enum(['realistic', 'cartoon', 'abstract', 'photorealistic']).optional(),
...
const imageFileSchema = z.object({
  file: z.object({
    mimetype: z.string().refine(
      (type) => ['image/png', 'image/jpeg', 'image/jpg', 'image/gif', 'image/bmp', 'image/webp'].includes(type),
      'Invalid image format. Only PNG, JPEG, GIF, BMP, and WebP are allowed.'
    ),
    size: z.number().max(10 * 1024 * 1024, 'Image file too large. Maximum size is 10MB.'),
  }).optional(),
});
