...
const checkUsageLimit = catchAsync(async (req, res) => {
  ...
  // Accept common aliases and map them to internal limit keys
  const typeMap = {
    webSearch: 'webSearch',
    websearch: 'webSearch',
    search: 'webSearch',
    ...
    exadeepresearch: 'deepResearch',
    monitor: 'monitor',
    examonitor: 'monitor',
  };

  const limitType = typeMap[rawType] || null;
...
const incrementUsage = catchAsync(async (req, res) => {
  const userId = req.user._id;
  const rawType = (req.body.limitType || '').toString();
  const typeMap = {
    webSearch: 'webSearch',
    websearch: 'webSearch',
    search: 'webSearch',
