...
const writingTask = async (req, res) => {
  const { message, conversationId } = req.body;
    ...
    try {
      ...
      const result = await writingAssistantApp.invoke(inputs, { configurable: { thread_id: conversationId } });
      ...
      if (result.finalContent) {
        ...
        res.json({ responseMessage: result.responseMessage, thread_id: thread_id });
      }
    } catch (error) {
