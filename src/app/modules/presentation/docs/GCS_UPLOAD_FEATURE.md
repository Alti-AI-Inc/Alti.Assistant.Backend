...
The module uses the `inso_assistant_presentation` bucket (configurable via `GCS_PRESENTATION_BUCKET` env var).

**Bucket Structure:**

```
inso_assistant_presentation/
  ├── {userId}/
  │   ├── {conversationId}/
...
```json
{
  ...
  "publicUrl": "https://storage.googleapis.com/inso_assistant_presentation/user123/conv456/presentation_abc123.pptx",
  "creditsConsumed": 10
}
```
...
```javascript
{
  metadata: {
    // ... other metadata
    presentationUrl: "https://storage.googleapis.com/inso_assistant_presentation/.../presentation.pptx",
