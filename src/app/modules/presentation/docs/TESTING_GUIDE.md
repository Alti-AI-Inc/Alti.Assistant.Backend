...
1. **Google Cloud Storage Setup**
   ...
   ```bash
   # Ensure inso_gcp.json credentials file exists
   ls inso_gcp.json

   # Verify GCS bucket exists or create it
   gsutil ls gs://inso_assistant_presentation || gsutil mb gs://inso_assistant_presentation
   ```
...
2. **Environment Configuration**
   ...
   GCS_PRESENTATION_BUCKET=inso_assistant_presentation
   ```
...
2. ✅ Verify URL format: `https://storage.googleapis.com/inso_assistant_presentation/...`
...
```json
{
  ...
  "data": {
    ...
    "publicUrl": "https://storage.googleapis.com/inso_assistant_presentation/direct_api/direct_1234567890/presentation_abc123.pptx",
    "credits_consumed": 10
  }
}
```
...
1. Temporarily rename `inso_gcp.json` to break GCS credentials
...
gsutil ls -r gs://inso_assistant_presentation/

# Expected structure:
# gs://inso_assistant_presentation/user_id_1/conversation_id_1/presentation_abc123.pptx
# gs://inso_assistant_presentation/user_id_1/conversation_id_1/presentation_def456_edited.pptx
# gs://inso_assistant_presentation/direct_api/direct_1234567890/presentation_xyz789.pptx
```

### Using GCS Console

1. Go to [Google Cloud Console](https://console.cloud.google.com/storage)
2. Navigate to `inso_assistant_presentation` bucket
...
1. Check `inso_gcp.json` exists and has correct permissions
2. Verify `GCP_PROJECT_ID` in `.env`
3. Check bucket exists: `gsutil ls gs://inso_assistant_presentation`
...
1. Check file permissions: `gsutil iam get gs://inso_assistant_presentation`
2. Make bucket publicly readable:
   ```bash
   gsutil iam ch allUsers:objectViewer gs://inso_assistant_presentation
   ```
3. Verify file is public: `gsutil acl get gs://inso_assistant_presentation/path/to/file.pptx`
...
gsutil -m rm -r gs://inso_assistant_presentation/test_*
gsutil -m rm -r gs://inso_assistant_presentation/direct_api/
