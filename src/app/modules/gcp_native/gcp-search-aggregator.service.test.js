...
const mockWebResultsPage1 = {
  data: {
    items: [
      { title: 'Result 1', link: 'http://example.com/1', displayLink: 'example.com/1', snippet: 'Snippet for result 1 about Alti Assistant', formattedUrl: 'http://example.com/1' },
  ...
  describe('executeRawSearch', () => {
    ...
    it('should execute a web search and format results correctly', async () => {
      ...
      expect(results[0]).toEqual({
        ...
        snippet: 'Snippet for result 1 about Alti Assistant',
  ...
  describe('executeParallelSearch', () => {
    ...
    it('should generate sub-queries, execute searches, deduplicate, score, and rank results', async () => {
      const mockResultsSubQuery1 = [
        { title: 'Alti Assistant platform overview', link: 'http://example.com/alti-platform', snippet: 'The Alti Assistant platform is great.', source: 'google_web' },
      ...
      const mockResultsSubQuery2 = [
        { title: 'News about Alti Assistant', link: 'http://example.com/alti-news', snippet: 'Recent news for the platform.', source: 'google_web' },
        { title: 'Common result', link: 'http://example.com/common', snippet: 'This appears in multiple searches.', source: 'google_web' },
      ];
      const mockResultsSubQuery3 = [
        { title: 'Alti Assistant specifications', link: 'http://example.com/alti-specs', snippet: 'Core details of the platform.', source: 'google_web' },
      ];
      ...
      const response = await GcpSearchAggregatorService.executeParallelSearch('what is the Alti Assistant platform', 'web', 5);

      expect(executeRawSearchSpy).toHaveBeenCalledTimes(3);
      expect(executeRawSearchSpy).toHaveBeenCalledWith('what is the Alti Assistant platform', 'web', 10, 1, 'active');
      expect(executeRawSearchSpy).toHaveBeenCalledWith('the Alti Assistant platform recent news analysis', 'web', 10, 1, 'active');
      expect(executeRawSearchSpy).toHaveBeenCalledWith('the Alti Assistant platform core details specifications', 'web', 10, 1, 'active');

      expect(response.success).toBe(true);
      expect(response.originalQuery).toBe('what is the Alti Assistant platform');
      ...
      // 'Alti Assistant platform overview' -> title: alti, platform (2*10), snippet: alti, platform (2*2) -> score 24
      // 'News about Alti Assistant' -> title: alti (10), snippet: platform (2) -> score 12
      // 'Alti Assistant specifications' -> title: alti (10), snippet: platform (2) -> score 12
      // 'Common result' -> title: 0, snippet: 0 -> score 0
      expect(response.results[0].title).toBe('Alti Assistant platform overview');
      expect(response.results[0].relevanceScore).toBe(26);
      expect(response.results[0].finalRank).toBe(1);

      expect(response.results[1].title).toBe('News about Alti Assistant');
      expect(response.results[1].relevanceScore).toBe(14);
      expect(response.results[1].finalRank).toBe(2);

      expect(response.results[2].title).toBe('Alti Assistant specifications');
    ...
    it('should correctly generate sub-queries for short queries (<= 2 words)', async () => {
        ...
        await GcpSearchAggregatorService.executeParallelSearch('Alti Assistant');

        expect(executeRawSearchSpy).toHaveBeenCalledWith('Alti Assistant', expect.any(String), expect.any(Number), expect.any(Number), expect.any(String));
        expect(executeRawSearchSpy).toHaveBeenCalledWith('Alti Assistant latest updates', expect.any(String), expect.any(Number), expect.any(Number), expect.any(String));
        expect(executeRawSearchSpy).toHaveBeenCalledWith('Alti Assistant overview details', expect.any(String), expect.any(Number), expect.any(Number), expect.any(String));
    });
