...
const groundedPromptResponse = async (sessionId, prompt, userId) => {
  ...
  try {
    ...
    // Call modern Gemini AI with active search grounding and gemini-3.1-pro-preview reasoning engine
    logger.info(`Sending prompt with live Google Search Grounding using gemini-3.1-pro-preview: "${prompt.slice(0, 50)}..."`);
    ...
    const responseData = {
      prompt,
      model: 'gemini-3.1-pro-preview-grounded',
      reply,
      groundingMetadata,
