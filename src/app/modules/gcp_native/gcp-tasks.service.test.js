...
describe('GcpTasksService', () => {
  ...
  beforeEach(() => {
    ...
    mockClientRequest.mockResolvedValue({
      data: {
        name: 'projects/test-project-id/locations/us-central1/queues/inso-default-tasks/tasks/test-task-id',
  ...
  describe('createHttpTask', () => {
    ...
    it('should successfully create a task with default parameters', async () => {
      const result = await GcpTasksService.createHttpTask('inso-default-tasks', defaultUrl);
      ...
      expect(mockClientRequest).toHaveBeenCalledWith({
        url: 'https://cloudtasks.googleapis.com/v2/projects/test-project-id/locations/us-central1/queues/inso-default-tasks/tasks',
        method: 'POST',
        data: {
          task: {
            httpRequest: {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        'GCP Cloud Tasks: Dispatching task to queue "projects/test-project-id/locations/us-central1/queues/inso-default-tasks"...'
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
    ...
    it('should successfully create a task with a payload', async () => {
      const result = await GcpTasksService.createHttpTask('inso-default-tasks', defaultUrl, defaultPayload);
    ...
    it('should successfully create a task with a delay', async () => {
      ...
      const result = await GcpTasksService.createHttpTask('inso-default-tasks', defaultUrl, {}, delaySeconds);
    ...
    it('should successfully create a task with custom headers', async () => {
      ...
      const result = await GcpTasksService.createHttpTask('inso-default-tasks', defaultUrl, {}, 0, customHeaders);
    ...
    it('should throw an error if GCP Project ID is not configured', async () => {
      ...
      await expect(GcpTasksService.createHttpTask('inso-default-tasks', defaultUrl)).rejects.toThrow(
    ...
    it('should throw an error if target callback URL is not provided', async () => {
      await expect(GcpTasksService.createHttpTask('inso-default-tasks', undefined)).rejects.toThrow(
    ...
    it('should prioritize config.google.gcp_project_id over process.env.GCP_PROJECT_ID', async () => {
      ...
      await GcpTasksService.createHttpTask('inso-default-tasks', defaultUrl);
    ...
    it('should handle null payload gracefully (no body in request)', async () => {
      await GcpTasksService.createHttpTask('inso-default-tasks', defaultUrl, null);
