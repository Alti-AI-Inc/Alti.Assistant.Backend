...
const tools = [
  ...
  { type: "function", function: { name: "query_schemafree_drill", description: "Use the Aphura Schema-Free SQL Engine (Apache Drill, Apache 2.0) to execute ANSI SQL queries directly over unstructured JSON, Parquet, and MongoDB data without pre-defining schemas.", parameters: { type: "object", properties: { query: { type: "string" }, storagePath: { type: "string" } }, required: ["query"] } } },
  ...
  { type: "function", function: { name: "access_unified_storage", description: "Use the Aphura Universal Storage Layer (Apache OpenDAL, Apache 2.0) to read, write, and stream data across 30+ storage backends (MinIO, S3,HDFS, Redis) with a single unified async API.", parameters: { type: "object", properties: { storageBackend: { type: "string" }, objectPath: { type: "string" } }, required: ["storageBackend", "objectPath"] } } },
  ...
  { type: "function", function: { name: "run_autogen_debate", description: "Use the Aphura Multi-Agent Debate Engine (Microsoft AutoGen, 36k stars, MIT) to launch collaborative cross-examining conversations between specialized AI agents. Beats standard models on planning.", parameters: { type: "object", properties: { taskGoal: { type: "string" }, agentRoles: { type: "string" } }, required: ["taskGoal"] } } },
  ...
  { type: "function", function: { name: "manage_cognitive_memory", description: "Use the Aphura Cognitive Memory Layer (Mem0, 25k stars, Apache 2.0) to store and recall long-term user preferences, decisions, and facts across all sessions and platforms. Beats standard memory.", parameters: { type: "object", properties: { userId: { type: "string" }, sessionFact: { type: "string" } }, required: ["userId", "sessionFact"] } } },
  ...
  { type: "function", function: { name: "scale_ray_compute", description: "Use the Aphura Distributed Cluster Engine (Ray, 34k stars, Apache 2.0) to distribute heavy Python tasks, simulations, and parallel processing across Liberty Center One bare-metal nodes.", parameters: { type: "object", properties: { taskName: { type: "string" }, taskCount: { type: "number" } }, required: ["taskName"] } } },
  ...
  { type: "function", function: { name: "write_cassandra_batch", description: "Use the Aphura Masterless NoSQL Database (Apache Cassandra, 9.5k stars, Apache 2.0) to execute high-velocity, multi-rack distributed writes with zero single point of failure.", parameters: { type: "object", properties: { keyspace: { type: "string" }, table: { type: "string" } }, required: ["keyspace", "table"] } } },
  ...
  { type: "function", function: { name: "query_timeseries_questdb", description: "Use the Aphura Time-Series Engine (QuestDB, 14k stars, Apache 2.0) to run vectorized sub-millisecond aggregations on high-frequency financial tick and telemetry data.", parameters: { type: "object", properties: { symbol: { type: "string" }, timeFrame: { type: "string" } }, required: ["symbol"] } } },
  { type: "function", function: { name: "get_online_features", description: "Use the Aphura Feature Store (Feast, Apache 2.0) to serve low-latency machine learning feature vectors in <1ms without train/serve skew.", parameters: { type: "object", properties: { entityId: { type: "string" }, featureNames: { type: "string" } }, required: ["entityId"] } } },
  ...
  { type: "function", function: { name: "sync_mobile_watermelon", description: "Use the Aphura Reactive Mobile Store (WatermelonDB, 10k stars, MIT) for high-performance offline-first local database sync across iOS, Android, and Desktop apps.", parameters: { type: "object", properties: { userId: { type: "string" }, clientChanges: { type: "string" } }, required: ["userId"] } } },
  { type: "function", function: { name: "calculate_spatial_gis", description: "Use the Aphura Geospatial Engine (Turf.js, 8.5k stars, MIT) for spatial analysis, polygon clustering, geofences, and drive-time calculations natively.", parameters: { type: "object", properties: { clusterRadiusKm: { type: "number" } }, required: ["clusterRadiusKm"] } } },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Use the Aphura Sovereign Object Storage (MinIO) to store files, images, PDFs, datasets, and model checkpoints in an S3-compatible bucket that runs 100% on Liberty Center One.",
  ...
  },
  {
    ...
    function: {
      ...
      description: "Use the Aphura DevOps Engine (Pulumi) to autonomously write and deploy Infrastructure-as-Code to Liberty Center One.",
