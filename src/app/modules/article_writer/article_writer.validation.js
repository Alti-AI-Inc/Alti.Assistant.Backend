...
const conversationalRequestSchema = z.object({
  body: z
    .object({
      ...
      // The userId field is validated if present but then removed via transform.
      // This allows for potential client-side tracking (e.g., for guest users) but ensures
      // the backend exclusively uses a server-managed identity (e.g., from req.user)
      // to prevent any possibility of user impersonation via the request body, thus isolating user data.
      userId: z.string().uuid('Invalid user ID format.').optional(),
      ...articleParameters,
    })
    .transform((data) => {
      const { userId, ...rest } = data; // eslint-disable-line no-unused-vars
      return rest;
    }),
});
  ...
  body: z
    .object({
      ...
      content: z
        ...
        .optional(),
      // The userId field is validated but then removed. See conversationalRequestSchema for explanation.
      userId: z.string().uuid('Invalid user ID format.').optional(),
      ...articleParameters,
    })
