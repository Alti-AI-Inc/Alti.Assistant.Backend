...
import ConversationModel from '../conversations/conversation.model.js'; // Assuming ConversationModel exists at this path
...
export const conversationalAssistant = catchAsync(async (req, res) => {
  ...
  // BUG FIX: Removed the line below to prevent IDOR vulnerability.
  // The userId should always be derived from the authenticated user's token (req.user)
  // or generated by the system for guests, not taken from req.body.
  ...
  if (!isGuest) {
    // OPTIMIZATION: Use .lean() for this read-only query to improve performance by returning a plain JavaScript object instead of a full Mongoose document.
    // INDEXING_RECOMMENDATION: Add a compound index on { userId: 1, createdAt: -1 } to the SubscriptionModel schema for faster lookups and sorting.
    const userSubscription = await SubscriptionModel.findOne({ userId })
      .sort({
        createdAt: -1,
      })
      .lean();
    const monthlyLimit = userSubscription ? userSubscription.usage : 0; // Assuming 'usage' field in SubscriptionModel is the monthly limit.

    // BUG FIX: The previous logic for checking subscription limits was flawed.
    // It incorrectly compared the monthly limit against either 0 or a conversation object,
    // which did not represent the user's actual usage for the current month.
    // This new logic correctly calculates the user's current month's usage
    // and compares it against the monthly limit from their subscription.
    const currentMonthStart = new Date(
      new Date().getFullYear(),
      new Date().getMonth(),
      1
    );
    // INDEXING_RECOMMENDATION: Ensure ConversationModel has a compound index on { userId: 1, createdAt: 1 } for efficient counting of user conversations within a date range.
    ...
    // Removed the problematic `totalConversationWithConvId` variable and its associated `if` condition.
  }
  ...
  try {
    ...
    // (e.g., on `userId`, `conversationId`, `createdAt` for conversation messages).
...
export const getConversationHistory = catchAsync(async (req, res) => {
  ...
  try {
    ...
    // on { conversationId: 1, userId: 1, createdAt: 1 } for efficient retrieval and sorting of a specific user's conversation.
