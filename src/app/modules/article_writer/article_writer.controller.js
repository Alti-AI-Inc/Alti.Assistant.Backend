...
export const conversationalAssistant = catchAsync(async (req, res) => {
  ...
  if (!isGuest) {
    ...
    // Recommendation: Ensure ConversationModel has an index on { userId: 1, createdAt: 1 } for efficient counting.
...
export const getConversationHistory = catchAsync(async (req, res) => {
  ...
  try {
    ...
    // and has appropriate indexes (e.g., on { conversationId: 1, userId: 1 } and { createdAt: 1 })
    // on the underlying conversation message model for efficient retrieval and sorting.
