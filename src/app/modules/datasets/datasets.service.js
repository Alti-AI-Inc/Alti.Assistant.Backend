...
const getGcsBucket = () => {
  try {
    const keyPath = config.google.google_application_credentials || path.join(process.cwd(), 'inso_gcp.json');
    const storage = new Storage({ keyFilename: keyPath });
    // Default bucket to one configured in env, or a standard inso dataset storage bucket
    const bucketName = config.gcs.knowledge_bank_bucket || 'inso_assistant_datasets';
