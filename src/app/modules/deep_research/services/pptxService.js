 ...
 * @param {object} workspace - The workspace object, containing subscription and usage data for authorization.
 * @param {object} workspace.subscription - The subscription details for the workspace.
 * @param {'pro'|'enterprise'} workspace.subscription.plan - The subscription plan name.
 * @param {'active'|'inactive'|'past_due'} workspace.subscription.status - The status of the subscription.
 * @param {object} [workspace.subscription.usage] - Usage metrics for the current billing period.
 * @param {number} [workspace.subscription.usage.reportsGeneratedThisMonth=0] - The number of reports generated this month.
 ...
 * @throws {Error} If the workspace does not have a valid, active subscription.
 * @throws {Error} If the workspace's plan does not support the feature.
 * @throws {Error} If the workspace has exceeded its monthly usage limit for reports.
 */
export const generatePPTXReport = async (reportDataParam, workspace) => {
  // --- Subscription & Limit Verification ---
  // This is a premium feature. Verify the workspace has an active, valid subscription and is within usage limits.
  if (!workspace || !workspace.subscription || workspace.subscription.status !== 'active') {
    // Throw a specific error for clear client-side handling.
    throw new Error('A valid, active subscription is required to generate reports.');
  }

  const { plan, usage } = workspace.subscription;
  // Define plan-based limits and features (e.g., watermarking).
  const planLimits = {
    'pro': { monthlyReports: 10, watermark: true },
    'enterprise': { monthlyReports: Infinity, watermark: false }
  };

  const currentPlanLimits = planLimits[plan];

  if (!currentPlanLimits) {
    throw new Error(`The '${plan}' plan does not include premium PPTX report generation.`);
  }

  // Check monthly usage limit against the plan's allowance.
  const reportsThisMonth = usage?.reportsGeneratedThisMonth || 0;
  if (reportsThisMonth >= currentPlanLimits.monthlyReports) {
    throw new Error('You have reached your monthly report generation limit.');
  }
  // Note: In a real application, the usage count should be incremented in the database *after* successful generation.
  // e.g., `await workspaceService.incrementReportCount(workspace.id);`
  ...
  // --- Presentation Constants & Theme ---
  pptx.layout = 'LAYOUT_16x9'; // Set modern 16:9 widescreen layout.
  
  const FONT_FACE_BODY = 'Arial';
  ...
  const HEADER_TEXT = 'ALTI ASSISTANT | ENTERPRISE DEEP RESEARCH STRATEGIC BRIEFING';
  const TOTAL_SLIDES = 4;

  // --- Reusable Slide Component Helpers ---
  const addHeaderAndFooter = (slide, pageNum) => {
    // Header Branding
    slide.addText(HEADER_TEXT, {
      x: 0.5, y: 0.4, w: 9.0, h: 0.3, fontSize: 10, bold: true, color: COLOR_TEXT_MUTED, fontFace: FONT_FACE_BODY
    });
    // Footer page slide branding
    slide.addText(`CONFIDENTIAL | GOOGLE CLOUD ENTERPRISE AI STRATEGY BRIEFING DECK • SLIDE ${pageNum} OF ${TOTAL_SLIDES}`, {
      x: 0.5, y: 7.1, w: 12.3, h: 0.2, fontSize: 7, color: COLOR_TEXT_MUTED, align: 'center', fontFace: FONT_FACE_BODY
    });
  };

  const addWatermark = (slide) => {
    if (currentPlanLimits.watermark) {
      slide.addText('Generated by Alti.Assistant', {
        x: '80%', y: '90%', w: '20%', h: '10%',
        ...
        align: 'right',
        valign: 'bottom',
        isTextBox: true, // Ensures text stays within bounds
      });
    }
  };
  ...
  slide1.addText(title || 'Executive Strategy Briefing Dashboard', {
    x: 0.5, y: 0.7, w: 9.0, h: 0.5, fontSize: 24, bold: true, color: COLOR_DARK_SLATE, fontFace: FONT_FACE_BODY
  });
  
  // Subtitle / Objective Context Box
  slide1.addShape(pptx.shapes.RECTANGLE, { x: 0.5, y: 1.3, w: 12.3, h: 0.6, fill: { color: 'F1F5F9' } });
  slide1.addText(`Objective: "${query || 'No specific query provided'}"`, {
    x: 0.7, y: 1.4, w: 11.9, h: 0.4, fontSize: 11, italic: true, color: '334155', fontFace: FONT_FACE_BODY
  });
  ...
  slide1.addText('KEY EXECUTIVE TAKEAWAYS', {
    x: 0.8, y: 2.3, w: 5.3, h: 0.3, fontSize: 12, bold: true, color: COLOR_DARK_SLATE, fontFace: FONT_FACE_BODY
  });
  ...
  slide1.addText(takeawayBullets.map(b => `• ${b}`).join('\n\n'), {
    x: 0.8, y: 2.7, w: 5.3, h: 3.0, fontSize: 10, color: '334155', fontFace: FONT_FACE_BODY, lineSpacing: 16
  });
  ...
  slide1.addText('RESEARCH RIGOR SCORECARD', {
    x: 7.0, y: 2.3, w: 5.5, h: 0.3, fontSize: 12, bold: true, color: COLOR_DARK_SLATE, fontFace: FONT_FACE_BODY
  });
  
  const qMetrics = metadata?.qualityMetrics || {};
  const metricsList = [
  ...
  metricsList.forEach(m => {
    slide1.addText(m.label, { x: 7.0, y: scoreY, w: 2.2, h: 0.3, fontSize: 9.5, bold: true, color: '475569', fontFace: FONT_FACE_BODY });
    ...
    slide1.addText(`${m.val.toFixed(1)}/10`, { x: 11.7, y: scoreY, w: 0.8, h: 0.3, fontSize: 9.5, bold: true, color: COLOR_TEAL, fontFace: FONT_FACE_BODY });
    scoreY += 0.7;
  });
  ...
  slide1.addText('GOLD STANDARDS VERIFIED STATISTIC', {
    x: 0.7, y: 6.2, w: 11.9, h: 0.2, fontSize: 8.5, bold: true, color: COLOR_TEAL, fontFace: FONT_FACE_BODY
  });
  ...
  slide1.addText(goldFact.value || 'N/A', {
    x: 0.7, y: 6.4, w: 1.5, h: 0.5, fontSize: 24, bold: true, color: COLOR_TEAL, fontFace: FONT_FACE_BODY
  });
  
  slide1.addText(`"${(goldFact.metric || '').replace('_____', ' ')}" - Verified in: ${goldFact.source || 'N/A'}`, {
    x: 2.2, y: 6.45, w: 10.2, h: 0.4, fontSize: 9.5, color: COLOR_DARK_SLATE, fontFace: FONT_FACE_BODY
  });
  ...
  const tableRows = [
    [
      { text: 'Metric Description', options: { bold: true, color: COLOR_WHITE, fill: { color: COLOR_DARK_SLATE }, align: 'left', fontFace: FONT_FACE_BODY, fontSize: 10 } },
      { text: 'Value', options: { bold: true, color: COLOR_WHITE, fill: { color: COLOR_DARK_SLATE }, align: 'center', fontFace: FONT_FACE_BODY, fontSize: 10 } },
      { text: 'Reference Source', options: { bold: true, color: COLOR_WHITE, fill: { color: COLOR_DARK_SLATE }, align: 'left', fontFace: FONT_FACE_BODY, fontSize: 10 } },
      { text: 'Trust Level', options: { bold: true, color: COLOR_WHITE, fill: { color: COLOR_DARK_SLATE }, align: 'center', fontFace: FONT_FACE_BODY, fontSize: 10 } },
      { text: 'Score', options: { bold: true, color: COLOR_WHITE, fill: { color: COLOR_DARK_SLATE }, align: 'center', fontFace: FONT_FACE_BODY, fontSize: 10 } }
    ]
  ];
  
  let displayFacts = facts.slice(0, 6);
  ...
  displayFacts.forEach((fact, idx) => {
    ...
    tableRows.push([
      { text: fact.metric || '', options: { fill: { color: rowBg }, align: 'left', fontSize: 9, fontFace: FONT_FACE_BODY, color: '334155' } },
      { text: fact.value || '', options: { fill: { color: rowBg }, align: 'center', fontSize: 9.5, bold: true, color: COLOR_TEAL, fontFace: FONT_FACE_BODY } },
      { text: fact.source || '', options: { fill: { color: rowBg }, align: 'left', fontSize: 8.5, color: COLOR_TEXT_MUTED, fontFace: FONT_FACE_BODY } },
      { text: (fact.trustLevel || 'MEDIUM').toUpperCase(), options: { fill: { color: rowBg }, align: 'center', fontSize: 8.5, bold: true, color: fact.trustLevel === 'HIGH' ? '16A34A' : (fact.trustLevel === 'LOW' ? '64748B' : 'D97706'), fontFace: FONT_FACE_BODY } },
      { text: fact.verificationScore ? `${fact.verificationScore}%` : '85%', options: { fill: { color: rowBg }, align: 'center', fontSize: 9, color: '334155', fontFace: FONT_FACE_BODY } }
    ]);
  });
  ...
  debateBubbles.forEach((bubble, idx) => {
    const accentColor = idx === 0 ? COLOR_TEAL : (idx === 1 ? COLOR_GOLD : '6366F1');
    const bgColor = idx === 0 ? 'F0FDF4' : (idx === 1 ? 'FFFBEB' : 'EEF2FF');
    const textColor = idx === 0 ? '16A34A' : (idx === 1 ? 'D97706' : '4F46E5');

    slide3.addShape(pptx.shapes.RECTANGLE, { x: bubbleX, y: 1.6, w: 3.8, h: 4.8, fill: { color: COLOR_WHITE } });
    slide3.addShape(pptx.shapes.RECTANGLE, { x: bubbleX, y: 1.6, w: 3.8, h: 0.08, fill: { color: accentColor } });
    
    slide3.addShape(pptx.shapes.ROUNDED_RECTANGLE, { x: bubbleX + 0.3, y: 1.9, w: 3.2, h: 0.4, fill: { color: bgColor } });
    slide3.addText(bubble.speaker, {
      x: bubbleX + 0.3, y: 1.95, w: 3.2, h: 0.3, fontSize: 9.5, bold: true, align: 'center', color: textColor, fontFace: FONT_FACE_BODY
    });
    
    slide3.addText(`"${bubble.quote}"`, {
      x: bubbleX + 0.3, y: 2.6, w: 3.2, h: 3.5, fontSize: 10, color: '334155', fontFace: FONT_FACE_BODY, lineSpacing: 16, italic: true
    });
  ...
  slide4.addText('RECOMMENDED ENTERPRISE ROADMAP', {
    x: 0.8, y: 1.8, w: 6.6, h: 0.3, fontSize: 12, bold: true, color: COLOR_DARK_SLATE, fontFace: FONT_FACE_BODY
  });
  ...
  slide4.addText(recommendations.map(r => `• ${r}`).join('\n\n'), {
    x: 0.8, y: 2.3, w: 6.6, h: 3.8, fontSize: 10.5, color: '334155', fontFace: FONT_FACE_BODY, lineSpacing: 18
  });
  ...
  slide4.addText('Briefing Index Summary', {
    x: 8.5, y: 1.9, w: 3.9, h: 0.4, fontSize: 16, bold: true, color: COLOR_WHITE, fontFace: FONT_FACE_BODY
  });
  ...
  slide4.addText('This strategic briefing slide deck was compiled automatically from recursive deep research datasets gathered across verified network references and consensus board audits.\n\nAll metrics are fully traceable back to primary source domains and verified by modern grounding LLMs in offline-safe environments.', {
    x: 8.5, y: 2.5, w: 3.9, h: 3.5, fontSize: 10, color: 'CBD5E1', fontFace: FONT_FACE_BODY, lineSpacing: 16
  });
  
  // --- Finalization & Export ---
  // Export presentation to node buffer. This can throw an error if pptxgenjs fails.
  ...
  const timestampStr = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
