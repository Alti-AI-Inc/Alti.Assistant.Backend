...
export const breadthFirstSearchNode = async (state) => {
    ...
    try {
        ...
        console.log(`Generated ${searchQueries.length} search queries for breadth-first search`);
        ...
        for (const query of searchQueries) {
            ...
            if (searchResult.results) {
                allSources.push(...searchResult.results.map(r => ({
                    title: r.title,
                    url: r.url,
...
Format your response as a JSON array of lead objects.`;
        ...
        const leadsAnalysis = await runGroqTask(
            analysisPrompt,
            [{ role: "user", content: "Please analyze these results and identify promising research leads." }],
            false
        );
        ...
        let promisingLeads = [];
        try {
            const cleanAnalysis = leadsAnalysis.replace(/```json\s*|\s*```/g, '').trim();
        ...
        const knowledgeGraph = await buildKnowledgeGraph(originalQuery, breadthResults, promisingLeads);
        
        console.log(`Identified ${promisingLeads.length} promising leads for deep research`);
...
export const deepDiveResearchNode = async (state) => {
    ...
    try {
        ...
        for (const [index, lead] of promisingLeads.entries()) {
            console.log(`Deep diving into lead ${index + 1}/${promisingLeads.length}: ${lead.title}`);
            ...
            for (const query of deepQueries) {
                ...
                if (searchResult.results) {
                    leadResults.sources.push(...searchResult.results.map(r => ({
                        title: r.title,
                        url: r.url,
                        snippet: r.content || r.snippet,
            ...
            leadResults.analysis = await synthesizeLeadFindings(lead, leadResults.deepSearches, originalQuery);
            
            deepDiveResults.push(leadResults);
        }
        
        console.log(`Deep dive research completed for all ${promisingLeads.length} leads`);
        ...
        return {
            ...
            metadata: {
                totalSearches: (state.metadata?.totalSearches || 0) + deepDiveResults.reduce((acc, lead) => acc + lead.deepSearches.length, 0)
            }
        };
    } catch (error) {
...
export const synthesizeComprehensiveReportNode = async (state) => {
    ...
    const { originalQuery, breadthResults, promisingLeads, deepDiveResults, knowledgeGraph, allSources } = state;
...
Use markdown formatting, include proper citations [1], [2], etc., and ensure the report is comprehensive yet readable.`;
        ...
        const finalReport = await runGroqTask(
            synthesisPrompt,
            [{ role: "user", content: "Please create the comprehensive research report based on all the collected information." }],
            false
        );
        ...
        return {
            ...
            finalReport: "Error: Failed to synthesize comprehensive report. Please try again."
        };
    }
};
...
export const saveDeepResearchNode = async (state) => {
    ...
    const { originalQuery, finalReport, qualityMetrics, metadata, deepDiveResults, allSources } = state;
...
export const generateDeepResearchPDFNode = async (state) => {
    ...
    const { originalQuery, finalReport, allSources, generatePdf, metadata, qualityMetrics } = state;
...
const buildKnowledgeGraph = async (originalQuery, breadthResults, promisingLeads) => {
...
const calculateCredibilityScore = (sources) => {
    ...
    const highCredibilityDomains = [
        ...
        'acm.org', 'pubmed.ncbi.nlm.nih.gov', 'scholar.google.com'
    ];
...
const calculateOverallConfidence = (qualityMetrics) => {
    const { sourceDiversity, informationDepth, topicCoverage, credibilityScore } = qualityMetrics;
    return (sourceDiversity + informationDepth + topicCoverage + credibilityScore) / 4;
};
...
const extractLeadsFromText = async (text, originalQuery) => {
    ...
    return [
        ...
        },
        {
            ...
            researchQuestions: [`What are the recent developments in ${originalQuery}?`]
        },
        {
            title: `${originalQuery} - Technical Details`,
            whyPromising: "Deep technical understanding",
            researchQuestions: [`What are the technical aspects of ${originalQuery}?`]
        }
    ];
};
