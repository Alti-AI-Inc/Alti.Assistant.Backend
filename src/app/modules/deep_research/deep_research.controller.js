...
export const performDeepResearch = catchAsync(async (req, res) => {
  ...
  try {
    ...
    if (process.env.RESEARCH_AGENT_URL) {
      ...
      logger.warn(
        '[DEEP_RESEARCH] RESEARCH_AGENT_URL is not set. Using local Gemini fallback mode.'
      );
...
- If specific real-time data is uncertain, clearly say assumptions are based on general knowledge.`;
      ...
      let fallbackNote =
        'Research agent was not configured; response generated by local fallback.';
      ...
      try {
        ...
        logger.warn(
          '[DEEP_RESEARCH] Gemini fallback failed. Using deterministic emergency fallback.',
          {
            isQuotaError,
            error: errorText,
          }
        ...
        fallbackAnswer =
          `Research request received: "${message}"\n\n` +
          `Current limitation:\n` +
