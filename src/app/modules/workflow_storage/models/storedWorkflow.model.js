...
storedWorkflowSchema.virtual('isExecutable').get(function() {
  return this.status === 'ready' && (!this.missingConnections || this.missingConnections.length === 0);
});
...
storedWorkflowSchema.statics.searchWorkflows = function(userId, searchTerm, options = {}) {
  const {
    limit = 20,
    offset = 0
  } = options;
...
storedWorkflowSchema.methods.updateConnections = function(connectedAccounts) {
  ...
  this.missingConnections = this.requiredApps.filter(app => !connectedAppSlugs.includes(app));
