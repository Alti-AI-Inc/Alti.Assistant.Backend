...
export const ETF_NAME_MAP = {
  ...
  '3x sp500': 'UPRO',
  soxl: 'SOXL',
  '3x semiconductor': 'SOXL',
  soxs: 'SOXS',
  'inverse semiconductor': 'SOXS',
  // International ETFs
  vxus: 'VXUS',
  international: 'VXUS',
  vea: 'VEA',
...
export const FOREX_NAME_MAP = {
  ...
  'dollar yen': 'USDJPY',
  yen: 'USDJPY',
  'yen to dollar': 'USDJPY',
  'dollar to yen': 'USDJPY',
  'japanese yen': 'USDJPY',
  usdchf: 'USDCHF',
  'usd/chf': 'USDCHF',
  'dollar franc': 'USDCHF',
  ...
  rupiah: 'USDIDR',
  usdmyr: 'USDMYR',
  'malaysian ringgit': 'USDMYR',
  ringgit: 'USDMYR',
  usdpkr: 'USDPKR',
  'pakistani rupee': 'USDPKR',
  usdngn: 'USDNGN',
  'nigerian naira': 'USDNGN',
  naira: 'USDNGN',
  usdegp: 'USDEGP',
  'egyptian pound': 'USDEGP',
  usdkes: 'USDKES',
  'kenyan shilling': 'USDKES',
  shilling: 'USDKES',
  usdghc: 'USDGHS',
  'ghanaian cedi': 'USDGHS',
  cedi: 'USDGHS',
  usdpen: 'USDPEN',
  'peruvian sol': 'USDPEN',
  peruvian: 'USDPEN',
...
export function detectFinancialIntent(prompt) {
  ...
  if (hasWatchlistKeyword || (allTickers.length >= 3 && !hasCompare)) {
    if (allTickers.length > 0) {
      return {
        type: 'watchlist',
        symbols: allTickers.map((m) => m.symbol),
        tickers: allTickers,
      };
    }
  }
  ...
  const groupEntries = Object.entries(GROUP_STOCK_MAP).sort(
  ...
  if (MOVERS_KEYWORDS.some((k) => q.includes(k))) {
    ...
    return {
      type: 'movers',
      direction: isLosers ? 'losers' : isActive ? 'active' : 'gainers',
    };
  }

  // 1g-2. Short interest as a standalone top-level intent (most shorted, short squeeze candidates)
  const shortInterestWithoutTicker =
    q.includes('most shorted') ||
    q.includes('short squeeze candidate') ||
    q.includes('heavily shorted stocks');
  ...
  for (const [name, proxy] of Object.entries(INDEX_MAP)) {
    if (q.includes(name))
      return { type: 'index', symbol: proxy, indexName: name };
  }

  // 4. Commodities — use ETF proxies
  for (const [name, proxy] of Object.entries(COMMODITY_MAP)) {
    if (q.includes(name))
      return { type: 'commodity', symbol: proxy, commodityName: name };
  }
  ...
  const hasShortInterestKeyword = SHORT_INTEREST_KEYWORDS.some((k) =>
    q.includes(k)
  );

  // 6e. Crypto technical check (for BTC/ETH-specific technical queries)
  const hasCryptoTechnicalKeyword = CRYPTO_TECHNICAL_KEYWORDS.some((k) =>
    q.includes(k)
  );
  ...
  const portfolioMatches = [
    ...prompt.matchAll(
      /(?:(\d+)\s+shares?\s+(?:of\s+)?([A-Z]{1,5}))|(?:(\d+)\s+([A-Z]{2,5})\b)/g
    ),
  ];
  const hasPortfolioIntent =
    portfolioMatches.length >= 2 &&
  ...
  const parsePeriod = () => {
    if (q.includes('1 day') || q.includes('today'))
      return { days: 1, timespan: 'hour', label: '1 Day' };
    if (
      q.includes('1 week') ||
      ...
      return { days: 7, timespan: 'day', label: '7 Days' };
    if (
      q.includes('1 month') ||
      ...
      return { days: 30, timespan: 'day', label: '30 Days' };
    if (q.includes('3 month') || q.includes('quarter') || q.includes('90 day'))
      return { days: 90, timespan: 'day', label: '3 Months' };
    if (q.includes('6 month') || q.includes('half year'))
      return { days: 180, timespan: 'week', label: '6 Months' };
    if (
      q.includes('1 year') ||
      q.includes('ytd') ||
      ...
      return { days: 365, timespan: 'week', label: '1 Year' };
    if (q.includes('5 year'))
      return { days: 1825, timespan: 'month', label: '5 Years' };
  ...
  const hasOptionsContractKeyword = optionsContractKeywords.some((k) =>
    q.includes(k)
  );
  ...
  const hasDividendCalendarKeyword = dividendCalendarKeywords.some((k) =>
    q.includes(k)
  );

  // 6f. Currency conversion with amount: "convert 1000 EUR to USD", "how much is 500 GBP in JPY"
  const convertMatch = prompt.match(
    /(?:convert|exchange|how much is)\s+([\d,]+(?:\.\d+)?)\s+([A-Z]{3})\s+(?:to|in|into)\s+([A-Z]{3})/i
  );
  if (convertMatch) {
  ...
  if (dollarMatch) {
    ...
    if (hasDividendCalendarKeyword)
      return { type: 'dividend_calendar', symbol: sym };
    if (hasChartKeyword)
      return {
        type: 'chart',
        symbol: sym,
    ...
    if (hasOptionsContractKeyword)
      return { type: 'options_contract', symbol: sym };
  ...
  if (bareMatch) {
    ...
    if (allTickers.has(sym)) {
      ...
      if (has52WeekKeyword) return { type: 'week52', symbol: sym };
      if (hasDividendKeyword) return { type: 'dividend', symbol: sym };
      if (hasShortInterestKeyword)
        return { type: 'short_interest', symbol: sym };
      return { type: 'stock', symbol: sym };
    }
  }
  ...
  const hasWordMatch = (name) => {
    // Avoid accidental matches for extremely short or ambiguous words
    ...
    const hasFinancialContext =
      /\b(stock|share|price|ticker|trading|market|invest|buy|sell|earnings|ipo|etf|fund|option|call|put|crypto|coin|forex|currency|rate|yield|equity|portfolio|dividend|index|gold|silver|oil)\b/i.test(
        prompt
      );

    if (name.length <= 2 && !hasFinancialContext) {
      return false;
    }

    // If the name is an ambiguous common word (e.g. "match"), require explicit financial context
    if (ambiguousWords.has(name.toLowerCase()) && !hasFinancialContext) {
      return false;
    }
  ...
  for (const [name, sym] of Object.entries(CRYPTO_NAME_MAP)) {
    if (hasWordMatch(name)) {
      ...
      if (hasCryptoTechnicalKeyword || hasTechnicalKeyword)
        return { type: 'crypto_technical', symbol: sym };
      return { type: 'crypto', symbol: sym };
    }
  }

  // 10. Forex lookup (word-boundary, longer phrases first to avoid substring collisions)
  const forexEntries = Object.entries(FOREX_NAME_MAP).sort(
    (a, b) => b[0].length - a[0].length
  );
...
export function detectMultipleTickers(prompt) {
  ...
  // Stock names
  for (const [name, sym] of Object.entries(STOCK_NAME_MAP)) {
    // Use word-boundary check to avoid substring collisions (e.g., 'ma' inside 'match')
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const rx = new RegExp(`\\b${escaped}\\b`, 'i');
    if (rx.test(q) && !found.find((f) => f.symbol === sym)) {
...
export function detectAllTickers(prompt) {
  ...
  const sortedStockNames = Object.entries(STOCK_NAME_MAP).sort(
    (a, b) => b[0].length - a[0].length
  );
  ...
  const sortedEtfNames = Object.entries(ETF_NAME_MAP).sort(
    (a, b) => b[0].length - a[0].length
  );
  ...
  const sortedCryptoNames = Object.entries(CRYPTO_NAME_MAP).sort(
    (a, b) => b[0].length - a[0].length
  );
