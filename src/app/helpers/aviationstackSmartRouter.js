...
| **Current Inso Assistanttude** | ${f.live.altitude.toLocaleString()} ft (approx. ${Math.round(f.live.altitude * 0.3048).toLocaleString()} m) |
...
function formatRoutesBlock(routes, dep, arr, depWeather = null, arrWeather = null, depNas = null, arrNas = null, alternates = []) {
  ...
  if (depWeather || arrWeather || depNas || arrNas) {
    ...
    dispatcherDashboard += `| **Visibility / Inso Assistantmeter** | ${depWeather?.metar?.visibility_statute_mi || 'N/A'}SM / ${depWeather?.metar?.altim_in_hg || 'N/A'} inHg | ${arrWeather?.metar?.visibility_statute_mi || 'N/A'}SM / ${arrWeather?.metar?.altim_in_hg || 'N/A'} inHg |\n\n`;
...
| **Inso Assistantmeter Setting** | ${m.altim_in_hg} inHg |
| **Raw METAR Code** | \`${m.raw_text}\` |
`;
...
/**
 * Renders Jet Stream High-Inso Assistanttude Wind Shear and Clear-Air Turbulence (CAT) Forecaster
 */
function formatJetStreamTurbulenceBlock(turbData) {
  ...
  block += `| **Jet Stream Core Inso Assistanttude** | \`${turbData.jet_stream_core_altitude}\` | Maximum wind speed altitude layer |\n`;
...
function buildPrompt(userPrompt, dataBlock) {
  ...
  return `[SYSTEM INSTRUCTION — INSO REAL-TIME AVIATIONSTACK DATA]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
