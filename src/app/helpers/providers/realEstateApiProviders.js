...
function parseRealEstateMetadata(markdown, fallbackTopic) {
  const addressMatch =
    markdown.match(/\*\*Address\*\*:\s*\*?\*?([^\n|]+?)\*?\*?/i) ||
    markdown.match(/Address\s*:\s*([^\n|]+)/i);
  const valuationMatch =
    markdown.match(/\*\*Current AVM\*\*\s*\|\s*\*?\*?(\$[0-9,]+)\*?\*?/i) ||
    markdown.match(/Valuation:\s*\*?\*?(\$[0-9,]+)\*?\*?/i);
  const rangeMatch =
    markdown.match(
      /range:\s*\*?\*?(\$[0-9,]+)\*?\*?\s*to\s*\*?\*?(\$[0-9,]+)\*?\*?/i
    ) ||
    markdown.match(
      /\|\s*\*?\*?(\$[0-9,]+)\*?\*?\s*-\s*\*?\*?(\$[0-9,]+)\*?\*?\s*\|/i
    );
  const limitMatch = markdown.match(/Limit\s*\|\s*\*?\*?(\$[0-9,]+)\*?\*?/i);

  return {
    domain: 'real_estate',
    address: addressMatch
      ? addressMatch[1].trim()
      : fallbackTopic || 'Property',
    valuation: valuationMatch ? valuationMatch[1] : null,
    lowRange: rangeMatch ? rangeMatch[1] : null,
    highRange: rangeMatch ? rangeMatch[2] : null,
    conformingLimit: limitMatch ? limitMatch[1] : null,
...
export const RealEstateApiProvider = {
  ...
  mandatoryRule:
    '▸ Present property metrics, valuations, and historical comps in standard Markdown tables',
  ...
  detectIntent: (query) => {
    try {
      ...
      logger.error(
        `[RealEstateProvider] Intent detection crash: ${err.message}`
      );
      return false;
    }
  },
  ...
  extractTopic: (query) => {
    try {
      ...
      const address =
        intent.entities?.address || intent.entities?.propertyId || 'Miami FL';
  ...
  fetch: async (topic, originalQuery) => {
    ...
    logger.info(
      `[RealEstateProvider] Executing live query via realestateSmartRouter: "${query.substring(0, 50)}..."`
    );
    ...
    try {
      ...
      const conformingLimitMatch = markdown.match(
        /Limit\s*\|\s*\*?\*?(\$[0-9,]+)/i
      );
      ...
      const conformingLimit = conformingLimitMatch
        ? conformingLimitMatch[1]
        : '$766,550';
