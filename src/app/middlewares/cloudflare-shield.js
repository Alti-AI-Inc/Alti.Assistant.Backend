 ...
 * Liberty Center One should NEVER be accessed directly. All traffic MUST flow through Cloudflare.
...
export const CloudflareShield = {
  async initialize() {
    try {
      ...
      logger.info(`[Cloudflare Shield] Armed with ${cloudflareIps.length} Cloudflare subnets. Liberty Center One is now a Dark Backend.`);
    } catch (error) {
  ...
  verifyRequest(req, res, next) {
    ...
    if (!ipRangeCheck(clientIp, cloudflareIps)) {
      ...
      return res.status(403).json({ 
        error: 'Access Denied', 
        message: 'Direct access to Liberty Center One private cloud is strictly forbidden. Traffic must route through the verified OEM Edge.'
      });
    }
