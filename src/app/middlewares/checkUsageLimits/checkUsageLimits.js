...
export const checkUsageLimits = async (userId, session = null) => {
  ...
  const promptsUsed =
    usage && typeof usage.promptsUsed === 'number' ? usage.promptsUsed : 0;
  const imagesUsed =
    usage && typeof usage.imagesUsed === 'number' ? usage.imagesUsed : 0;

  let errors = [];

  if (promptsUsed >= planLimits.prompts) {
    errors.push(
      `Your ${subObj.plan_name || 'explore'} plan prompts limit reached.`
    );
  }
