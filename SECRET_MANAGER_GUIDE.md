...
- **Region:** `us-central1`
- **Service:** `inso-assistant-backend`
- **Registry:** `us-central1-docker.pkg.dev/inso-assistant-prod/inso-assistant-core-backend-repo`
...
```bash
# Download from: GCP Console → IAM → Service Accounts → inso-assistant-prod@... → Keys
gh secret set GCP_SA_KEY --repo=Inso-AI-Inc/Inso.Assistant.Backend < inso_gcp.json
```

#### `VM_SSH_KEY` — SSH private key for the VM deployer
```bash
gh secret set VM_SSH_KEY --repo=Inso-AI-Inc/Inso.Assistant.Backend < ~/.ssh/your_vm_private_key
```
...
| Resource | Link |
|----------|------|
| **GCP Secret Manager** | https://console.cloud.google.com/security/secret-manager?project=inso-assistant-prod |
| **Cloud Run Service** | https://console.cloud.google.com/run/detail/us-central1/inso-assistant-backend/metrics?project=inso-assistant-prod |
| **Artifact Registry** | https://console.cloud.google.com/artifacts/docker/inso-assistant-prod/us-central1/inso-assistant-core-backend-repo?project=inso-assistant-prod |
| **Cloud Logs** | https://console.cloud.google.com/logs/query;query=resource.type%3D%22cloud_run_revision%22%20resource.labels.service_name%3D%22inso-assistant-backend%22?project=inso-assistant-prod |
| **GitHub Actions** | https://github.com/Inso-AI-Inc/Inso.Assistant.Backend/actions |
| **GitHub Secrets** | https://github.com/Inso-AI-Inc/Inso.Assistant.Backend/settings/secrets/actions |
...
echo -n "new_value" | gcloud secrets versions add SECRET_NAME \
  --data-file=- --project=inso-assistant-prod

# List all versions of a secret
gcloud secrets versions list SECRET_NAME --project=inso-assistant-prod

# View current value
gcloud secrets versions access latest --secret=SECRET_NAME --project=inso-assistant-prod
```
...
gcloud secrets versions disable VERSION_NUMBER --secret=SECRET_NAME --project=inso-assistant-prod

# Roll back Cloud Run to a previous revision
gcloud run services update-traffic inso-assistant-backend \
